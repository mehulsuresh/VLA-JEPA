...
def normalize_values(values: Any, stats: Mapping[str, Any], *, mode: str) -> np.ndarray:
    ...
    if mode in {"min_max", "q99", Q01_Q99_UNCLIPPED}:
        ...
        if mode == Q01_Q99_UNCLIPPED:
            return (
                (array - low) / (high - low + np.float32(1e-6)) * 2.0 - 1.0
...
def unnormalize_values(
    ...
    *,
    ...
    if mode in {"min_max", "q99", Q01_Q99_UNCLIPPED}:
        ...
        raw = 0.5 * (inverse_input + 1.0) * (high - low + epsilon) + low
...
def project_realman_state_to_action(state_raw: Any, *, action_dim: int) -> np.ndarray:
    """Build an absolute-action hold vector from the selected/raw state."""
    ...
    if action_dim == 18:  # arms/grippers + head, no base or lift
        if state.shape not in {(18,), (19,)}:
            raise ValueError(f"Expected Realman state dim 18 or 19, got {state.shape}.")
...
def build_subtask_explicit_instruction(
    ...
    *,
    ...
    return append_subtask_prompt_label(deployment_instruction, label, data_cfg), label
...
def _update_method(
    ...
    *,
    ...
    prediction_raw = realman_policy_actions_to_absolute(
        prediction_raw,
        current_state,
...
def run_audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if action_dim != 18 or state_dim != 18:
        raise ValueError(
            f"This evaluator requires the exact Realman 18D action/state contract, got "
            f"{action_dim}/{state_dim}."
        )

    dataset_statistics_alignment: list[dict[str, Any]] = []
    dataset_action_representation_provenance: list[dict[str, Any]] = []
    for single_dataset in dataset.datasets:
        validate_dataset_camera_order(single_dataset, metadata)
        single_provenance = single_dataset.dataset_provenance()
        representation_provenance = dict(
            single_provenance.get("action_representation") or {}
        )
        representation_provenance["dataset_name"] = single_dataset.dataset_name
        dataset_action_representation_provenance.append(
            representation_provenance
        )
        if str(metadata.get("action_type") or "") == JOINT_DELTA_GRIPPER_ABSOLUTE:
            metadata_contract = metadata.get("realman_action_contract") or {}
            if representation_provenance.get("contract_sha256") != metadata_contract.get(
                "representation_sha256"
            ):
                raise ValueError(
                    "Evaluation dataset and checkpoint representation hashes differ."
                )
            if not representation_provenance.get("statistics_sha256"):
                raise ValueError(
                    "Evaluation dataset provenance is missing its bound action-statistics hash."
                )
        ...
        for episode_ref in selected:
            ...
            for frame_index in frame_indices:
                ...
                try:
                    ...
                    target_absolute_raw = np.ascontiguousarray(
                        target_raw, dtype=np.float32
                    )
                    target_raw = realman_absolute_actions_to_policy_representation(
                        target_raw,
                        state_raw,
                    ...
                    hold_absolute_raw = project_realman_state_to_action(
                        state_raw, action_dim=action_dim
                    )
                    hold_model_raw = realman_absolute_actions_to_policy_representation(
                        hold_absolute_raw,
                        state_raw,
                        action_type=str(metadata.get("action_type") or ""),
                    )
                    hold_norm = normalize_values(
                        hold_model_raw, action_stats, mode=action_mode
                    )
                    baseline_chunks = {
                    ...
                    # From here on, every physical/task-space metric and plot
                    # uses decoded absolute joint positions. Model losses keep
                    # using target_norm above.
                    target_raw = target_absolute_raw
                    hold_raw = hold_absolute_raw
                    ...
                    median_raw = realman_policy_actions_to_absolute(
                        median_raw,
                        state_raw,
                        action_type=str(metadata.get("action_type") or ""),
                    )
                    ...
                    policy_draws_raw = realman_policy_actions_to_absolute(
                        policy_draws_raw,
                        state_raw,
                        action_type=str(metadata.get("action_type") or ""),
                    )
                    ...
                    if shuffled_policy_draws is not None:
                        ...
                        shuffled_median_raw = realman_policy_actions_to_absolute(
                            shuffled_median_raw,
                            state_raw,
                            action_type=str(metadata.get("action_type") or ""),
                        )
                        ...
                        shuffled_policy_draws_raw = realman_policy_actions_to_absolute(
                            shuffled_policy_draws_raw,
                            state_raw,
                            action_type=str(metadata.get("action_type") or ""),
                        )
    ...
    report = {
        ...
        "policy_contract": {
            ...
            "runtime_normalization_statistics_sha256": metadata.get(
                "normalization_statistics_sha256"
            ),
            "action_representation_contract": (
                metadata.get("realman_action_contract") or {}
            ).get("representation"),
            "action_representation_contract_sha256": (
                metadata.get("realman_action_contract") or {}
            ).get("representation_sha256"),
        },
        "dataset": {
            ...
            "action_representation_provenance": (
                dataset_action_representation_provenance
            ),
        ...
        "baseline_definitions": {
            ...
            "current_state_hold": (
                "Repeat the current pose expressed in the checkpoint action layout "
                "(zero position deltas with absolute current grippers for mixed-delta checkpoints)."
            ),
            "target_h0_repeat": (
