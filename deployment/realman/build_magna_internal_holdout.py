#!/usr/bin/env python3
"""Build the immutable Magna train/holdout split and evaluation sampling plan.

The holdout is a deterministic SHA-256 ranking prefix over complete episodes.
Its episode count is configured independently from the evaluation observation
count. Windows are distributed as evenly as possible across heldout episodes,
with an explicit deterministic identity list for any division remainder. The
script also computes
normalization statistics from the train complement only.
"""
...
from starVLA.holdout_selection_contract import (  # noqa: E402
    DEFAULT_REALMAN_HOLDOUT_SELECTION_SEED_TEXT,
    REALMAN_HOLDOUT_RANKING_ALGORITHM,
...
DEFAULT_SEED_TEXT = DEFAULT_REALMAN_HOLDOUT_SELECTION_SEED_TEXT
RANKING_SCHEMA = REALMAN_HOLDOUT_RANKING_ALGORITHM
...
def _derive_effective_global_batch(
    ...
    *,
    ...
    configured_holdout_count = data_cfg.get("holdout_episode_count")
    configured_policy = data_cfg.get("holdout_sampling")
    if configured_holdout_count is not None and configured_policy is not None:
        raise ValueError(
    ...
    holdout_sampling_policy = (
        ...
        else validate_holdout_sampling_policy(configured_policy)
    )
    if holdout_sampling_policy is None:
        holdout_episode_count = (
            effective
            if configured_holdout_count is None
            else configured_holdout_count
        )
        if (
            ...
            raise ValueError(
                "datasets.vla_data.holdout_episode_count must be a positive integer"
            )
        if effective % holdout_episode_count != 0:
            raise ValueError(
                "The effective global batch must be an integer multiple of "
                "datasets.vla_data.holdout_episode_count: "
                f"{effective} % {holdout_episode_count} != 0"
            )
        evaluation_frames_per_episode = effective // holdout_episode_count
        evaluation_observation_count = effective
    else:
        holdout_episode_count = None
        evaluation_frames_per_episode = None
    ...
    return {
        ...
        "evaluation_frames_per_episode": (
            None
            if evaluation_frames_per_episode is None
            else int(evaluation_frames_per_episode)
        ),
...
def _supervised_window_eligibility(
    ...
    *,
    ...
    minimum_evaluable_windows: int = 1,
) -> tuple[np.ndarray, dict[int, str]]:
    """Mirror training's action mask and require enough supervised eval windows."""

    if (
        isinstance(minimum_evaluable_windows, bool)
        or not isinstance(minimum_evaluable_windows, int)
        or minimum_evaluable_windows <= 0
    ):
        raise ValueError("minimum_evaluable_windows must be a positive integer")
    ...
    structural_candidate_counts = np.maximum(
        lengths - (action_max - action_min),
        0,
    )
    length_eligible = structural_candidate_counts >= minimum_evaluable_windows

    def insufficient_structural_reason() -> str:
        if minimum_evaluable_windows == 1:
            return (
        ...
        return (
            f"fewer than {minimum_evaluable_windows} structurally unpadded "
            "evaluation windows spanning offsets "
            f"[{structural_window['required_min_offset']},"
            f"{structural_window['required_max_offset']}]"
        )
    ...
    if not bool(data_cfg.get("use_action_validity_prefix_mask", False)):
        return length_eligible, {
            int(row.episode_index): insufficient_structural_reason()
    ...
    for position, row in enumerate(episodes.itertuples(index=False)):
        episode_id = int(row.episode_index)
        length = int(row.length)
        if structural_candidate_counts[position] < minimum_evaluable_windows:
            reasons[episode_id] = insufficient_structural_reason()
            continue
        ...
        for base_index in range(valid_min, valid_max + 1):
            ...
            if bool(
                ...
                evaluable_windows += 1
                if evaluable_windows >= minimum_evaluable_windows:
                    break
        eligible[position] = evaluable_windows >= minimum_evaluable_windows
        if not eligible[position]:
            if minimum_evaluable_windows == 1:
                reasons[episode_id] = (
                    "no structurally unpadded action window has a supervised "
                    f"element under {label_key} sustained-invalid-prefix masking"
                )
            else:
                reasons[episode_id] = (
                    f"only {evaluable_windows} structurally unpadded action windows "
                    f"have a supervised element under {label_key} "
                    "sustained-invalid-prefix masking; "
                    f"{minimum_evaluable_windows} are required"
                )
...
def _compute_train_statistics(
    ...
    *,
    ...
    for field in schema:
        ...
        value_type = field.type.value_type if is_vector else field.type
...
def _reuse_existing_outputs(
    *,
    ...
    for key in (
        ...
        "launcher_sha256",
        "holdout_selection_contract_schema",
        "holdout_selection_contract_sha256",
        "holdout_selection_contract",
    ):
    ...
    if any(key in recorded_batch for key in explicit_sampling_keys):
        for key in explicit_sampling_keys:
            if recorded_batch.get(key) != batch[key]:
                raise ValueError(
                    f"Existing split manifest sampling binding {key} changed: "
                    f"{recorded_batch.get(key)!r} != {batch[key]!r}"
                )
    elif (
        batch["holdout_episode_count"] != batch["effective_global_batch_size"]
        or batch["base_frames_per_episode"] != 1
        or batch["extra_window_episode_count"] != 0
    ):
        raise ValueError(
            "Existing split manifest predates explicit multi-window holdout "
            "sampling and cannot be reused for this configuration"
        )
    for key in ("holdout_sampling_policy", "holdout_sampling_plan"):
        if recorded_batch.get(key) != batch.get(key):
            raise ValueError(
                f"Existing split manifest sampling binding {key} changed"
            )
    ...
    sampling = manifest.get("evaluation_sampling", {})
    if not isinstance(sampling, dict):
        raise ValueError("Existing split manifest evaluation sampling is invalid")
    allocation = sampling.get("window_allocation", {})
    expected_allocation = {
        "algorithm": batch["window_allocation_algorithm"],
        "holdout_episode_count": batch["holdout_episode_count"],
        "base_frames_per_episode": batch["base_frames_per_episode"],
        "extra_window_episode_count": batch["extra_window_episode_count"],
        "maximum_frames_per_episode": batch["maximum_frames_per_episode"],
        "extra_window_episode_identities": batch[
            "extra_window_episode_identities"
        ],
    }
    if (
        sampling.get("observation_count")
        != batch["evaluation_observation_count"]
        or allocation != expected_allocation
    ):
        raise ValueError(
            "Existing split manifest evaluation window allocation changed"
        )
    ...
    expected_report_contract = {
        "schema": batch["holdout_selection_contract_schema"],
        "sha256": batch["holdout_selection_contract_sha256"],
        "contract": batch["holdout_selection_contract"],
    }
    if (
        ...
        raise ValueError(
            "Existing split report has stale holdout selection provenance"
        )
...
def build(args: argparse.Namespace) -> None:
    ...
    holdout_selection_contract = build_realman_holdout_selection_contract(
        training_config,
        world_size=args.world_size,
        dataset_name=dataset_root.name,
        seed_text=args.seed_text,
    )
    batch["holdout_selection_contract_schema"] = (
        holdout_selection_contract["schema"]
    )
    batch["holdout_selection_contract_sha256"] = (
        holdout_selection_contract_sha256(holdout_selection_contract)
    )
    batch["holdout_selection_contract"] = holdout_selection_contract
    if batch["holdout_sampling_policy"] is not None:
        sampling_plan = derive_episode_holdout_sampling_plan(
            total_episode_count=len(episodes),
            evaluation_observation_count=int(
                batch["evaluation_observation_count"]
            ),
            policy=batch["holdout_sampling_policy"],
        )
        batch["holdout_episode_count"] = sampling_plan[
            "holdout_episode_count"
        ]
        batch["evaluation_frames_per_episode"] = (
            sampling_plan["base_frames_per_episode"]
            if sampling_plan["extra_window_episode_count"] == 0
            else None
        )
        for key in (
            "base_frames_per_episode",
            ...
            batch[key] = sampling_plan[key]
        batch["holdout_sampling_plan"] = sampling_plan
    else:
        batch["holdout_sampling_plan"] = None
    holdout_count = int(batch["holdout_episode_count"])
    if holdout_count >= len(episodes):
        raise ValueError("Configured holdout episode count leaves no training episodes")
    ...
    eligible_mask, ineligibility_reasons = _supervised_window_eligibility(
        ...
        minimum_evaluable_windows=int(batch["maximum_frames_per_episode"]),
    )
    ...
    if holdout_count > len(eligible_episodes):
        raise ValueError(
            f"Configuration requests {holdout_count} held-out episodes, "
            f"but only {len(eligible_episodes)} episodes can provide a supervised "
            f"set of {batch['maximum_frames_per_episode']} unique unpadded "
            "windows spanning offsets "
            f"[{structural_window['required_min_offset']}, "
            f"{structural_window['required_max_offset']}]."
        )
    ...
    extra_window_episode_identities = [
        ...
        for row in selected_ranked[: int(batch["extra_window_episode_count"])]
    ]
    batch["extra_window_episode_identities"] = (
        extra_window_episode_identities
    )
    ...
    manifest = {
        ...
        "description": (
            "A compact deterministic episode holdout with the exact "
            "config-owned checkpoint-evaluation observation count, balanced "
            "as evenly as possible across selected episodes."
        ),
        "role_contract": {
            "train_episode_selection": "complement_of_holdout",
            "evaluation_episode_selection": "holdout_episode_indices",
        ...
        "selection": {
            ...
            "eligible_episode_policy": (
                "complete episodes with enough distinct structurally unpadded "
                "deployment-observation/action windows containing at least one "
        ...
        "evaluation_sampling": {
            ...
            "observation_count": int(batch["evaluation_observation_count"]),
            "window_allocation": {
                "algorithm": str(batch["window_allocation_algorithm"]),
                "holdout_episode_count": int(batch["holdout_episode_count"]),
                "base_frames_per_episode": int(
                    batch["base_frames_per_episode"]
                ),
                "extra_window_episode_count": int(
                    batch["extra_window_episode_count"]
                ),
                "maximum_frames_per_episode": int(
                    batch["maximum_frames_per_episode"]
                ),
                "extra_window_episode_identities": (
                    extra_window_episode_identities
                ),
            ...
            **(
                {
                    # Compatibility alias for uniform plans. New readers bind
                    # the explicit allocation above; old readers can still
                    # consume manifests whose allocation has no remainder.
                    "frames_per_episode": int(
                        batch["base_frames_per_episode"]
                    )
                }
                if int(batch["extra_window_episode_count"]) == 0
                else {}
            ),
    ...
    report = {
        ...
        "evaluation_sampling": manifest["evaluation_sampling"],
