...
def model_compatible_observation(
    ...
    *,
    model_state_dim: int = REALMAN_SOURCE_STATE_DIM,
) -> dict[str, Any]:
    ...
    supplies the first 19 raw values so the payload builder can select the 18
    policy channels while preserving measured lift outside the model.
    """
...
class RealmanVrTeleopPolicyRobot:
    ...
    Observation path:
      ...
      -> raw 19D source state; the policy pipeline selects its trained 18D view
         and retains measured lift outside the model.
