...
REALMAN_STATE_NAMES = REALMAN_SOURCE_STATE_NAMES[:REALMAN_POLICY_DIM]
...
def realman_policy_action_state_indices(action_dim: int) -> np.ndarray:
    """Map policy channels to their state anchors; -1 means stay native."""

    action_dim = int(action_dim)
    realman_policy_action_names(action_dim)
    indices = np.full((action_dim,), -1, dtype=np.int64)
    indices[0:7] = np.arange(0, 7)
    indices[8:15] = np.arange(8, 15)
    if action_dim == REALMAN_ACTION_DIM:
        indices[19:21] = np.arange(16, 18)
        indices[21] = 18
    elif action_dim == REALMAN_POLICY_ACTION_DIM_NO_BASE:
        indices[16:19] = np.arange(16, 19)
    else:
        indices[16:18] = np.arange(16, 18)
    return indices


def _broadcast_realman_action_reference(
    actions: np.ndarray,
    current_state: Any,
    state_indices: np.ndarray,
) -> np.ndarray:
    state = np.asarray(current_state, dtype=np.float32)
    if state.shape[-1] != REALMAN_STATE_DIM:
        raise ValueError(
            f"RealMan state dim is {state.shape[-1]}, expected {REALMAN_STATE_DIM}."
        )
    reference = state[..., state_indices]
    while reference.ndim < actions.ndim:
        reference = np.expand_dims(reference, axis=-2)
    return reference


def realman_absolute_actions_to_policy_representation(
    actions: Any,
    current_state: Any,
    *,
    action_type: str,
) -> np.ndarray:
    """Convert absolute targets to the checkpoint's mixed action space."""

    array = np.asarray(actions, dtype=np.float32)
    if action_type != JOINT_DELTA_GRIPPER_ABSOLUTE:
        return np.ascontiguousarray(array)
    if array.shape[-1] != REALMAN_18D_ACTION_CONTRACT.action_dim:
        raise ValueError(
            "Delta checkpoints must use the exact versioned RealMan 18-D contract; "
            f"got action width {array.shape[-1]}."
        )
    return encode_actions(
        array,
        select_realman_policy_state(current_state),
        REALMAN_18D_ACTION_CONTRACT,
    )


def realman_policy_actions_to_absolute(
    actions: Any,
    current_state: Any,
    *,
    action_type: str,
) -> np.ndarray:
    """Restore state-relative joints before sending commands to the robot."""
...
def realman_continuous_unnormalize(
    ...
    *,
    ...
    if mode in {"q99", Q01_Q99_UNCLIPPED}:
        q01 = _stat("q01")
        q99 = _stat("q99")
        ...
        epsilon = np.float32(1e-6) if mode == Q01_Q99_UNCLIPPED else np.float32(0.0)
        return (0.5 * (array + 1.0) * (q99 - q01 + epsilon) + q01).astype(
            np.float32,
            copy=False,
        )
    if mode == "mean_std":
...
def extract_state_vector(observation: dict[str, Any]) -> np.ndarray:
    """Return the raw 19-D robot state, including measured lift height."""

    for key in _STATE_VECTOR_KEYS:
        if key in observation:
            state = _to_numpy(observation[key]).astype(np.float32, copy=False).reshape(-1)
            if state.shape[0] != REALMAN_SOURCE_STATE_DIM:
                raise ValueError(
                    f"Realman state vector `{key}` has dim {state.shape[0]}, "
                    f"expected raw source dim {REALMAN_SOURCE_STATE_DIM}."
                )
            return np.ascontiguousarray(state)

    if all(name in observation for name in REALMAN_SOURCE_STATE_NAMES):
        state = np.asarray(
            [observation[name] for name in REALMAN_SOURCE_STATE_NAMES],
            dtype=np.float32,
        )
        return np.ascontiguousarray(state)
    ...
    raise KeyError(
        ...
        f"{list(_STATE_VECTOR_KEYS)} with dim {REALMAN_SOURCE_STATE_DIM}, or provide all named state fields."
    )
...
def build_policy_payload(
    ...
    state = select_realman_policy_state(extract_state_vector(observation))
...
def validate_realman_policy_payload(
    ...
    *,
    require_normalized_bounds: bool | None = None,
) -> None:
    contract = metadata.get("realman_input_contract") or {}
    ...
    if require_normalized_bounds is None:
        require_normalized_bounds = bool(
            (metadata.get("realman_input_contract") or {}).get(
...
def validate_realman_server_metadata(
    metadata: dict[str, Any],
    *,
    expected_action_dim: int | Iterable[int] | None = REALMAN_POLICY_DIM,
    expected_state_dim: int = REALMAN_STATE_DIM,
    expected_action_type: str | None = JOINT_DELTA_GRIPPER_ABSOLUTE,
    ...
    action_contract = metadata.get("realman_action_contract")
    if metadata.get("action_type") == JOINT_DELTA_GRIPPER_ABSOLUTE and not isinstance(
        action_contract, dict
    ):
        warnings.append(
            "Server metadata does not include the required exact RealMan action contract."
        )
    if isinstance(action_contract, dict) and action_dim_ok:
        expected_state_indices = realman_policy_action_state_indices(
            int(reported_action_dim)
        ).tolist()
        if action_contract.get("action_type") != metadata.get("action_type"):
            warnings.append(
                "Server realman_action_contract.action_type does not match top-level action_type."
            )
        if metadata.get("action_type") == JOINT_DELTA_GRIPPER_ABSOLUTE:
            if action_contract.get("delta_anchor") != "chunk_start_state":
                warnings.append(
                    "Server RealMan delta action contract does not use chunk_start_state."
                )
            if action_contract.get("policy_action_state_indices") != expected_state_indices:
                warnings.append(
                    "Server RealMan action-to-state index mapping does not match the policy layout."
                )
            if action_contract.get("absolute_gripper_policy_action_indices") != [7, 15]:
                warnings.append(
                    "Server RealMan action contract does not preserve both grippers as absolute."
                )
            if int(reported_action_dim) == REALMAN_POLICY_DIM:
                if action_contract.get("representation_sha256") != REALMAN_18D_ACTION_CONTRACT.sha256():
                    warnings.append(
                        "Server RealMan action contract hash does not match the exact 18-D contract."
                    )
                if action_contract.get("representation") != REALMAN_18D_ACTION_CONTRACT.to_dict():
                    warnings.append(
                        "Server RealMan action contract payload does not match the exact 18-D contract."
                    )
                statistics_sha256 = metadata.get("normalization_statistics_sha256")
                if not isinstance(statistics_sha256, str) or len(statistics_sha256) != 64:
                    warnings.append(
                        "Server metadata is missing the normalization-statistics SHA-256."
                    )
                elif action_contract.get(
                    "normalization_statistics_sha256"
                ) != statistics_sha256:
                    warnings.append(
                        "Server action contract and top-level normalization-statistics hashes differ."
                    )
                for mode_key in (
                    "default_state_norm_mode",
                    "default_action_norm_mode",
                ):
                    if metadata.get(mode_key) != Q01_Q99_UNCLIPPED:
                        warnings.append(
                            f"Server {mode_key} must be {Q01_Q99_UNCLIPPED!r} for the exact delta contract."
                        )
    ...
    if require_input_contract and not isinstance(input_contract, dict):
        ...
        if metadata.get("action_type") == JOINT_DELTA_GRIPPER_ABSOLUTE:
            if input_contract.get(
                ...
                warnings.append(
                    "Server input contract hash does not match the exact 18-D representation."
                )
            if input_contract.get(
                "normalization_statistics_sha256"
            ) != metadata.get("normalization_statistics_sha256"):
                warnings.append(
                    "Server input contract and top-level normalization-statistics hashes differ."
                )
