...
- raw measured state: 19D `source.observation.state`; lift is retained only by
  the robot client
- model state: 18D `source.observation.state[0:18]`
- model action: 18D mixed targets: arm/head joint deltas relative to the
  current observation, with both grippers absolute
- action horizon: 50
...
The policy server reports this layout and its action-to-state index mapping in
`realman_action_contract`. After action unnormalization, the robot client adds
the measured current arm/head state back only on delta channels. Gripper values
remain absolute throughout. The client rejects incompatible action/state
dimensions or action representations.
...
- normalized state is finite `float32` with shape `[B, 1, 18]`
...
backbones are skipped by default. Keep the optional action-output retry
heuristic disabled for the alignment baseline; when enabled it reconstructs
absolute poses from the request state before applying its legacy magnitude
checks, but it still cannot determine whether a trajectory is safe or useful.
The robot client remains responsible for hard limits and per-command step
limits.
...
ordered camera frames; a finite 19D measured robot state; an 18D policy payload;
an 18D model output; a 22D expanded robot command; exactly zero base velocity;
and unchanged measured lift.
