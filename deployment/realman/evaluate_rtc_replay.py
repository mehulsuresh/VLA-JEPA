...
uint8 Qwen frames, the deployment-normalized 18D policy state, and one explicit task
...
from starVLA.action_representation import JOINT_DELTA_GRIPPER_ABSOLUTE
...
def _load_replay_observation(
    *,
    ...
    return ReplayObservation(
        ...
        state_raw=np.ascontiguousarray(state_raw, dtype=np.float32),
...
def _metrics_for_trace(
    ...
    *,
    ...
    state_raw_by_frame: Mapping[int, np.ndarray] | None = None,
    action_type: str = "",
    ...
    for execution in trace:
        ...
        state_raw = (
            state_raw_by_frame.get(execution.frame_index)
            if state_raw_by_frame is not None
            else None
        )
        if action_type == JOINT_DELTA_GRIPPER_ABSOLUTE and state_raw is None:
            raise ValueError(
                f"Missing chunk-start state for frame {execution.frame_index}."
            )
...
def run_replay(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if action_dim != 18 or state_dim != 18:
            raise ValueError(
                f"Sequential Realman replay requires the exact 18D action/state contract, "
                f"got {action_dim}/{state_dim}."
            )
        action_representation_provenance = dict(
            single_dataset.dataset_provenance().get("action_representation") or {}
        )
        checkpoint_representation = metadata.get("realman_action_contract") or {}
        if action_representation_provenance.get(
            "contract_sha256"
        ) != checkpoint_representation.get("representation_sha256"):
            raise ValueError(
                "Replay dataset and checkpoint action-representation hashes differ."
            )
        if not action_representation_provenance.get("statistics_sha256"):
            raise ValueError(
                "Replay dataset provenance is missing action-statistics SHA-256."
            )
        ...
        state_raw_by_frame: dict[int, np.ndarray] = {}
        ...
        for frame_index in frame_indices:
            ...
            state_raw_by_frame[frame_index] = observation.state_raw
        ...
        fresh_metrics, fresh_records = _metrics_for_trace(
            ...
            state_raw_by_frame=state_raw_by_frame,
        ...
        report = {
            ...
            "policy_contract": {
                ...
                "action_representation_contract": checkpoint_representation.get(
                    "representation"
                ),
                "action_representation_contract_sha256": checkpoint_representation.get(
            ...
            "replay": {
                ...
                "action_representation_provenance": action_representation_provenance,
