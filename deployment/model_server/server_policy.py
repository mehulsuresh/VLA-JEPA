...
from deployment.realman.pipeline import realman_policy_actions_to_absolute
...
class ActionGuardRetryPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        self.state_stats = state_stats_by_key.get(default_key) if default_key else None
        self.state_norm_mode = metadata.get("default_state_norm_mode") or "q99"
        self.action_type = str(metadata.get("action_type") or "")
    ...
    def predict_action(self, *args, **kwargs) -> dict[str, Any]:
        ...
        for attempt_index in range(self.max_attempts):
            ...
            valid, metrics, reasons = self._validate_output(
                output,
                normalized_state=kwargs.get("state"),
            )
            guard_record = {
                "enabled": True,
    ...
    def _validate_output(
        ...
        *,
        ...
        if self.action_type == "joint_delta_gripper_absolute":
            if normalized_state is None or self.state_stats is None:
                return False, {"shape": list(actions.shape)}, [
                    "mixed delta action guard requires normalized current state and state statistics"
                ]
            state_raw = _continuous_unnormalize(
                np.asarray(normalized_state, dtype=np.float32),
                self.state_stats,
                mode=self.state_norm_mode,
            )
            actions = realman_policy_actions_to_absolute(
                actions,
                state_raw,
