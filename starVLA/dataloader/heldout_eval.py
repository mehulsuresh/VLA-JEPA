...
episode at its explicitly allocated immutable, structurally valid frames. This module
...
that unbiased balanced-window view and can add a separate deterministic
...
class EvaluationSamplingContract:
    ...
    extra_window_episode_identities: tuple[tuple[str, int], ...]
    seed_sha256: str
    ...
    @property
    def frames_per_episode(self) -> int | None:
        """Legacy uniform multiplicity, or ``None`` for a balanced remainder."""

        if self.extra_window_episode_count:
            return None
        return self.base_frames_per_episode

    def windows_for_episode(self, dataset_name: str, episode_id: int) -> int:
        identity = (str(dataset_name), int(episode_id))
        return self.base_frames_per_episode + int(
            identity in self.extra_window_episode_identities
        )
...
def _realman_hold_raw_action(raw_state: np.ndarray, action_dim: int) -> np.ndarray:
    """Project the current RealMan state into an absolute-action hold.

    The legacy policy selected all 19 state channels.  The OpenPI-compatible
    policy intentionally selects only the 18 arm/gripper/head channels before
    transforms run.  Accepting both widths here keeps the baseline in raw
    physical units and lets the normal transform stack encode joint deltas and
    normalize it exactly like a policy target.
    """

    state = np.asarray(raw_state, dtype=np.float32).reshape(-1)
    if state.shape not in {(18,), (19,)}:
        raise ValueError(
            "Heldout RealMan baseline requires an 18D selected or 19D legacy "
            "raw state, "
            f"got {state.shape}."
        )
    ...
    if int(action_dim) == 19:
        if state.shape != (19,):
            raise ValueError(
                "A legacy 19D action hold requires the legacy 19D raw state."
            )
        return state.copy()
    if int(action_dim) == 22:
        if state.shape != (19,):
            raise ValueError(
                "A legacy 22D action hold requires the legacy 19D raw state."
            )
...
def _split_state_matrix(dataset: Any, values: np.ndarray) -> dict[str, np.ndarray]:
    state = np.asarray(values, dtype=np.float32)
    if state.ndim == 1:
        state = state[None, :]
    if state.ndim != 2:
        raise ValueError(
            "Heldout state transform requires [rows, state_dim], "
            f"got {state.shape}."
        )
    payload: dict[str, np.ndarray] = {}
    offset = 0
    for key in dataset.modality_keys.get("state", ()):
        if "." not in key:
            raise ValueError(f"Expected namespaced state key, got {key!r}.")
        subkey = key.split(".", 1)[1]
        metadata = getattr(dataset.metadata.modalities, "state")[subkey]
        shape = tuple(int(value) for value in metadata.shape)
        if len(shape) != 1 or shape[0] <= 0:
            raise ValueError(
                f"Heldout state metadata {key!r} must be one-dimensional, got {shape}."
            )
        next_offset = offset + shape[0]
        payload[key] = state[:, offset:next_offset]
        offset = next_offset
    if offset != state.shape[1]:
        raise ValueError(
            "Heldout state metadata dimensions do not match the selected state "
            f"dimension: metadata={offset}, state={state.shape[1]}."
        )
    return payload
...
def _normalize_action_matrix(
    ...
    *,
    ...
    if raw_state is not None:
        payload.update(_split_state_matrix(dataset, raw_state))
...
def _normalize_action_vector(
    dataset: Any,
    vector: np.ndarray,
    *,
    raw_state: np.ndarray | None = None,
) -> np.ndarray:
    normalized = _normalize_action_matrix(
        dataset,
        vector,
        raw_state=raw_state,
    )
...
def load_evaluation_sampling_contract(
    ...
    manifest_episode_identities: set[tuple[str, int]] = set()
    if datasets is not None:
        if not isinstance(datasets, list):
            raise ValueError("Episode split manifest datasets must be a list.")
        for entry in datasets:
            if not isinstance(entry, dict):
                raise ValueError("Episode split manifest dataset entries must be objects.")
            dataset_name = entry.get("dataset_name")
            episode_ids = entry.get("holdout_episode_indices")
            if not isinstance(dataset_name, str) or not dataset_name:
                raise ValueError("Manifest dataset_name must be a non-empty string.")
            if not isinstance(episode_ids, list):
                raise ValueError(
                    "Manifest holdout_episode_indices must be an integer list."
                )
            for episode_id in episode_ids:
                if isinstance(episode_id, bool) or not isinstance(episode_id, int):
                    raise ValueError(
                        "Manifest holdout_episode_indices must contain integers."
                    )
                identity = (dataset_name, int(episode_id))
                if identity in manifest_episode_identities:
                    raise ValueError(f"Duplicate heldout episode identity {identity!r}.")
                manifest_episode_identities.add(identity)

    allocation = sampling.get("window_allocation")
    if allocation is None:
        # Backward compatibility for immutable manifests generated before
        # balanced q/r allocation was introduced.
        ...
        if (
            ...
            raise ValueError(
                "In-training checkpoint evaluation requires a positive integer "
                f"frames_per_episode; got {frames_per_episode!r}."
            )
        holdout_episode_count = (
            len(manifest_episode_identities)
            if manifest_episode_identities
            else None
        )
        inferred_observation_count = (
            None
            if holdout_episode_count is None
            else holdout_episode_count * int(frames_per_episode)
        )
        observation_count = sampling.get(
        ...
        if observation_count is not None and (
            ...
            or (
                inferred_observation_count is not None
                and observation_count != inferred_observation_count
            )
        ):
            raise ValueError(
                "Legacy evaluation_sampling.observation_count must equal "
                "holdout episodes * frames_per_episode."
            )
        base_frames_per_episode = int(frames_per_episode)
        extra_window_episode_count = 0
        maximum_frames_per_episode = int(frames_per_episode)
        window_allocation_algorithm = "legacy_uniform_per_episode_v1"
        extra_window_episode_identities: tuple[tuple[str, int], ...] = ()
    else:
        if not isinstance(allocation, dict):
            raise ValueError("evaluation_sampling.window_allocation must be an object.")

        def _allocation_integer(name: str, *, minimum: int) -> int:
            ...
            if (
                ...
                raise ValueError(
                    f"evaluation_sampling.window_allocation.{name} must be "
                    f"an integer >= {minimum}; got {value!r}."
                )
            return int(value)

        observation_count = sampling.get("observation_count")
        if (
            isinstance(observation_count, bool)
            or not isinstance(observation_count, int)
            or observation_count <= 0
            ...
            raise ValueError(
                "Balanced evaluation_sampling.observation_count must be a "
                "positive integer."
            )
        holdout_episode_count = _allocation_integer(
            "holdout_episode_count", minimum=1
        )
        base_frames_per_episode = _allocation_integer(
            "base_frames_per_episode", minimum=1
        )
        extra_window_episode_count = _allocation_integer(
            "extra_window_episode_count", minimum=0
        )
        maximum_frames_per_episode = _allocation_integer(
            "maximum_frames_per_episode", minimum=1
        )
        window_allocation_algorithm = allocation.get("algorithm")
        if window_allocation_algorithm not in {
            "balanced_digest_rank_v1",
            "uniform_per_episode_v1",
        }:
            raise ValueError(
                "Unsupported evaluation window allocation algorithm "
                f"{window_allocation_algorithm!r}."
            )
        if extra_window_episode_count >= holdout_episode_count:
            raise ValueError(
                "extra_window_episode_count must be smaller than "
                "holdout_episode_count."
            )
        expected_maximum = base_frames_per_episode + int(
            extra_window_episode_count > 0
        )
        if maximum_frames_per_episode != expected_maximum:
            raise ValueError(
                "maximum_frames_per_episode does not match the balanced "
                "allocation."
            )
        ...
        if observation_count != expected_observations:
            raise ValueError(
                "evaluation_sampling observation count does not match "
                "holdout_episode_count * base_frames_per_episode + "
                "extra_window_episode_count."
            )
        ...
        if not isinstance(raw_extra_identities, list):
            raise ValueError(
                "extra_window_episode_identities must be a list of objects."
            )
        parsed_extra_identities: list[tuple[str, int]] = []
        for entry in raw_extra_identities:
            if not isinstance(entry, dict):
                raise ValueError(
                    "extra_window_episode_identities entries must be objects."
                )
            dataset_name = entry.get("dataset_name")
            ...
            if (
                ...
                raise ValueError(
                    "Each extra window identity requires a dataset_name string "
                    "and integer episode_id."
                )
            parsed_extra_identities.append((dataset_name, int(episode_id)))
        if (
            len(parsed_extra_identities) != extra_window_episode_count
            or len(set(parsed_extra_identities)) != len(parsed_extra_identities)
        ):
            raise ValueError(
                "extra_window_episode_identities must contain exactly "
                "extra_window_episode_count unique identities."
            )
        extra_window_episode_identities = tuple(parsed_extra_identities)
        if manifest_episode_identities:
            if holdout_episode_count != len(manifest_episode_identities):
                raise ValueError(
                    "window_allocation.holdout_episode_count does not match the "
                    "manifest-heldout episode identities."
                )
            unknown_extras = (
                set(extra_window_episode_identities)
                - manifest_episode_identities
            )
            if unknown_extras:
                raise ValueError(
                    "Extra-window identities are not heldout episodes: "
                    f"{sorted(unknown_extras)!r}."
                )
        legacy_frames = sampling.get("frames_per_episode")
        if legacy_frames is not None and (
            extra_window_episode_count != 0
            or legacy_frames != base_frames_per_episode
        ):
            raise ValueError(
                "frames_per_episode is only a valid compatibility alias for "
                "uniform window allocations."
            )
    ...
    return EvaluationSamplingContract(
        algorithm=algorithm,
        observation_count=(
            None if observation_count is None else int(observation_count)
        ),
        holdout_episode_count=(
            None
            if holdout_episode_count is None
            else int(holdout_episode_count)
        ),
...
def _candidate_control_diagnostics(
    *,
    ...
    for raw_step in candidates:
        ...
        raw_anchor_state = state_matrix[state_index]
        normalized_targets = _normalize_action_matrix(
            dataset,
            raw_targets,
            raw_state=raw_anchor_state,
        )
        if normalized_targets.shape != raw_targets.shape:
            raise ValueError(
                "Focused heldout normalized action chunk changed shape: "
                f"{normalized_targets.shape} != {raw_targets.shape}."
            )
        # The mixture packs normalized policy targets as float16 before
        # collation (see LeRobotMixtureDataset.__getitem__).  Movement coverage
        # and its hold denominator must use those exact target values or
        # threshold-adjacent elements can disagree with the runtime evaluator.
        normalized_targets = np.ascontiguousarray(
            normalized_targets.astype(np.float16).astype(np.float32),
            dtype=np.float32,
        )
        normalized_hold = _normalize_action_vector(
            dataset,
            raw_hold,
            raw_state=raw_anchor_state,
        )
...
def _uniform_candidate_indices(
    *,
    contract: EvaluationSamplingContract,
    dataset_name: str,
    episode_id: int,
    ...
    """Select a deterministic uniform-like subset without replacement."""

    requested = contract.windows_for_episode(dataset_name, episode_id)
    if candidate_count < requested:
        raise ValueError(
            f"Heldout episode {dataset_name}/{episode_id} has only "
            f"{candidate_count} evaluable windows, but the manifest requires "
            f"{requested} distinct windows for this episode."
        )
    if requested == 1:
        return (
            _uniform_candidate_index(
                contract=contract,
    ...
    ranked = sorted(
        ...
        key=lambda candidate_index: hashlib.sha256(
            _canonical_json_bytes(
                {
                    "algorithm": contract.algorithm,
                    "seed_sha256": contract.seed_sha256,
    ...
    return tuple(int(value) for value in ranked[:requested])
...
def select_heldout_window_references(
    ...
    *,
    ...
    """Select stable, distinct, unpadded windows from every heldout episode."""
    ...
    for dataset_index, dataset in enumerate(mixture.datasets):
        ...
        for episode_id in sorted(selected_ids):
            ...
            choices = _uniform_candidate_indices(
                contract=contract,
            ...
            for choice in choices:
                ...
                if (
                    ...
                    anchor_subtask_index = _coerce_optional_integer_label(
                        dataset.curr_traj_data.iloc[int(selected_step)][
                            "subtask_index"
                        ]
                    )
...
def select_focused_heldout_window_references(
    ...
    *,
    ...
    for (
        ...
        requested = contract.windows_for_episode(
            str(dataset.dataset_name), int(episode_id)
        )
        if len(base_pool) < requested:
            raise ValueError(
                f"Focused heldout episode {dataset.dataset_name}/{episode_id} "
                f"has only {len(base_pool)} evaluable windows, but the manifest "
                f"requires {requested} distinct windows for this episode."
            )
        remaining = list(base_pool)
        for _slot in range(requested):
            transition_pool = [
                item
            ...
            selection_pool = transition_pool or remaining
            ...
            def transition_balance(item: _CandidateDiagnostic) -> int:
                ...
                if not directions:
                    return len(episode_candidates) * requested + 1
                return min(
                    transition_window_counts[direction] for direction in directions
                )
            ...
            selected = min(
                ...
                key=lambda item: (
                    ...
                    else len(episode_candidates) * requested + 1,
...
def validate_global_eval_observation_count(
    *,
    ...
    """Validate immutable eval cardinality against its distributed eval batch.

    Explicit ``observation_count`` is the modern contract and is independent
    of training gradient accumulation. Legacy callers without that field keep
    the historical effective-training-batch validation.
    """
    ...
    if frames_per_episode is not None:
        values["frames_per_episode"] = frames_per_episode
    ...
    if observation_count is None:
        ...
        uniform_frames = 1 if frames_per_episode is None else int(frames_per_episode)
        observations = int(holdout_episode_count) * uniform_frames
        allocation_description = (
            f"holdout episodes={int(holdout_episode_count)} * "
            f"frames/episode={uniform_frames}"
        )
    else:
        ...
        if frames_per_episode is not None:
            uniform_observations = (
                int(holdout_episode_count) * int(frames_per_episode)
            )
            if observations != uniform_observations:
                raise ValueError(
                    "Explicit observation_count disagrees with the legacy "
                    "uniform frames_per_episode allocation."
                )
        distributed_eval_batch = (
            int(per_device_batch_size) * int(world_size)
        )
        if observations % distributed_eval_batch != 0:
            raise ValueError(
                "The explicit evaluation observation count must be divisible "
                "by the per-device eval batch and world size: "
                f"{observations} % ({int(per_device_batch_size)} * "
                f"{int(world_size)}) != 0."
            )
        allocation_description = f"explicit observations={observations}"
    if observations != expected:
        raise ValueError(
            "The legacy manifest sampling plan must contain exactly one "
            "effective global training batch: "
            f"{allocation_description} = {observations}, "
            f"expected={expected} "
...
class DeterministicHeldoutEvalDataset(LeRobotMixtureDataset):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            self.legacy_underfilled_eval
            and (
                sampling_contract.base_frames_per_episode != 1
                or sampling_contract.extra_window_episode_count != 0
            )
        ):
            raise ValueError(
                "Legacy underfilled heldout audit supports only "
        ...
        self.manifest_holdout_episode_count = sum(
            len(dataset.trajectory_ids) for dataset in self.datasets
        )
        self.original_manifest_observation_count = len(unbiased_references)
        if (
            sampling_contract.holdout_episode_count is not None
            and sampling_contract.holdout_episode_count
            != self.manifest_holdout_episode_count
        ):
            raise ValueError(
                "Evaluation sampling holdout_episode_count does not match the "
                "loaded manifest split."
            )
        if (
            sampling_contract.observation_count is not None
            and sampling_contract.observation_count
            != self.original_manifest_observation_count
        ):
            raise ValueError(
                "Evaluation sampling observation_count does not match the "
                "deterministically selected window count."
            )
        selected_identities = {
            (str(dataset.dataset_name), int(episode_id))
            for dataset in self.datasets
            for episode_id in dataset.trajectory_ids.tolist()
        }
        if not set(
            sampling_contract.extra_window_episode_identities
        ).issubset(selected_identities):
            raise ValueError(
                "Evaluation sampling extra-window identities are not present "
                "in the loaded heldout split."
            )
        ...
        invalid_reference_counts = {
            identity: count
            for identity, count in reference_counts.items()
            if count
            != sampling_contract.windows_for_episode(*identity)
        }
        if invalid_reference_counts:
            raise ValueError(
                "Selected heldout windows do not match the manifest q/r "
                f"allocation: {invalid_reference_counts!r}."
            )
        ...
        self.heldout_window_digest = _canonical_sha256(
            {
                ...
                "holdout_episode_count": (
                    sampling_contract.holdout_episode_count
                ),
                "window_allocation": {
                    "algorithm": (
                        sampling_contract.window_allocation_algorithm
                    ),
                    "base_frames_per_episode": (
                        sampling_contract.base_frames_per_episode
                    ),
                    "extra_window_episode_count": (
                        sampling_contract.extra_window_episode_count
                    ),
                    "maximum_frames_per_episode": (
                        sampling_contract.maximum_frames_per_episode
                    ),
                    "extra_window_episode_identities": [
                        {
                            "dataset_name": dataset_name,
                        ...
                        for dataset_name, episode_id in (
                            sampling_contract.extra_window_episode_identities
                        )
                    ],
                },
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        midpoint_raw = _raw_action_midpoint(dataset)
        if self.heldout_eval_action_dim == 18:
            # Statistics are for the already transformed mixed representation.
            # Joint/head extrema therefore describe deltas, not absolute
            # positions.  Only the two absolute gripper midpoints are needed;
            # make all delta channels an absolute hold so the normal transform
            # converts them back to zero before normalization.
            absolute_channels = np.asarray((7, 15), dtype=np.int64)
            midpoint_absolute = hold_raw.copy()
            midpoint_absolute[absolute_channels] = midpoint_raw[absolute_channels]
            midpoint_raw = midpoint_absolute
    ...
    def sampling_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "purpose": (
                (
                    ...
                    if (
                        self.evaluation_sampling_contract.base_frames_per_episode
                        == 1
                        and self.evaluation_sampling_contract
                        .extra_window_episode_count
                        == 0
                    )
                    else (
            ...
            "holdout_episode_count": self.manifest_holdout_episode_count,
            "base_frames_per_episode": (
                self.evaluation_sampling_contract.base_frames_per_episode
            ),
            "extra_window_episode_count": (
                self.evaluation_sampling_contract.extra_window_episode_count
            ),
            "maximum_frames_per_episode": (
                self.evaluation_sampling_contract.maximum_frames_per_episode
            ),
            "window_allocation_algorithm": (
                self.evaluation_sampling_contract.window_allocation_algorithm
            ),
            "extra_window_episode_identities": [
                {
                    "dataset_name": dataset_name,
                ...
                for dataset_name, episode_id in (
                    self.evaluation_sampling_contract
                    .extra_window_episode_identities
                )
            ],
        ...
        if self.evaluation_sampling_contract.frames_per_episode is not None:
            # Keep the legacy scalar only when every episode really has the
            # same multiplicity.
            report["frames_per_episode"] = (
                self.evaluation_sampling_contract.frames_per_episode
            )
        if self.legacy_underfilled_eval:
