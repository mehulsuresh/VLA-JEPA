...
from starVLA.action_representation import Q01_Q99_UNCLIPPED
...
class RealmanBimanualSourceDataConfig(TrossenAIStationaryDataConfig):
    ...
    def _anchor_relative_transform(self):
        data_cfg = getattr(self, "data_cfg", None)
        if data_cfg is None or str(data_cfg.get("action_type", "")) != "joint_delta_gripper_absolute":
            return None
        if str(data_cfg.get("action_delta_anchor", "chunk_start_state")) != "chunk_start_state":
            raise ValueError(
                "RealMan joint deltas require action_delta_anchor=chunk_start_state."
            )
        if str(data_cfg.get("gripper_action_type", "absolute")) != "absolute":
            raise ValueError(
                "joint_delta_gripper_absolute requires gripper_action_type=absolute."
            )
        mappings = data_cfg.get("action_delta_mappings", None)
        if not mappings:
            raise ValueError(
                "RealMan joint_delta_gripper_absolute requires action_delta_mappings."
            )
        plain_mappings = {
            str(key): dict(value.items()) if hasattr(value, "items") else dict(value)
            for key, value in mappings.items()
        }
        return AnchorRelativeActionTransform(
    ...
    def _normalization_mode(self) -> str:
        ...
        if data_cfg is None:
            return "min_max"
        configured = data_cfg.get("state_action_normalization", None)
        if configured is not None:
            return str(configured)
        if str(data_cfg.get("action_type", "")) == "joint_delta_gripper_absolute":
            return Q01_Q99_UNCLIPPED
        return "min_max"

    def transform(self):
        anchor_relative = self._anchor_relative_transform()
        normalization_mode = self._normalization_mode()
        transforms = [
            *([anchor_relative] if anchor_relative is not None else []),
            StateActionToTensor(apply_to=self.state_keys),
            StateActionTransform(
                apply_to=self.state_keys,
                normalization_modes={
                    "state.source": normalization_mode,
                },
            ),
            StateActionToTensor(apply_to=self.action_keys),
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={
                    "action.source": normalization_mode,
                },
            ),
        ]
    ...
    def transform(self):
        ...
        transforms = [
            *([anchor_relative] if anchor_relative is not None else []),
            ...
            StateActionTransform(
                apply_to=self.action_keys,
                normalization_modes={key: normalization_mode for key in self.action_keys},
            ),
        ]
