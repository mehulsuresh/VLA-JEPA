...
class EpisodeSplitSelection:
    manifest_path: Path
    manifest_sha256: str
    split_manifest_sha256_without_statistics_binding: str
    role: str
    ...
    action_representation_statistics_path: Path | None
    action_representation_statistics_sha256: str | None
    ...
    def provenance(self) -> dict[str, Any]:
        ...
        return {
            ...
            "split_manifest_sha256_without_statistics_binding": (
                self.split_manifest_sha256_without_statistics_binding
            ),
            "role": self.role,
            ...
            "action_representation_statistics_path": (
                str(self.action_representation_statistics_path)
                if self.action_representation_statistics_path is not None
                else None
            ),
            "action_representation_statistics_sha256": (
                self.action_representation_statistics_sha256
            ),
        }
...
def load_episode_split_selection(
    *,
    ...
    stable_split_manifest_sha256 = (
        split_manifest_sha256_without_statistics_binding(payload)
    )
    ...
    representation_statistics_path = None
    representation_statistics_sha256 = None
    raw_representation_statistics = entry.get("action_representation_statistics")
    if raw_representation_statistics is not None:
        if not isinstance(raw_representation_statistics, dict):
            raise ValueError(
                "action_representation_statistics must be a JSON object."
            )
        ...
        missing_representation_fields = sorted(
            required_representation_fields - set(raw_representation_statistics)
        )
        if missing_representation_fields:
            raise ValueError(
                "action_representation_statistics is missing required fields "
                f"{missing_representation_fields}."
            )
        raw_representation_path = raw_representation_statistics["path"]
        if not isinstance(raw_representation_path, str) or not raw_representation_path.strip():
            raise ValueError(
                "action_representation_statistics.path must be a non-empty string."
            )
        representation_statistics_path = Path(raw_representation_path).expanduser()
        if not representation_statistics_path.is_absolute():
            representation_statistics_path = path.parent / representation_statistics_path
        representation_statistics_path = representation_statistics_path.resolve()
        if not representation_statistics_path.is_file():
            raise FileNotFoundError(
                "Bound action-representation statistics file does not exist: "
                f"{representation_statistics_path}"
            )
        representation_statistics_sha256 = file_sha256(
            representation_statistics_path
        )
        if representation_statistics_sha256 != raw_representation_statistics["sha256"]:
            raise ValueError(
                "Action-representation statistics SHA-256 mismatch: expected "
                f"{raw_representation_statistics['sha256']!r}, got "
                f"{representation_statistics_sha256!r}."
            )
        if raw_representation_statistics["frame_count"] != train_frame_count:
            raise ValueError(
                "action_representation_statistics.frame_count does not match "
                "the train split."
            )
        if raw_representation_statistics["catalog_sha256"] != train_set_sha:
            raise ValueError(
                "action_representation_statistics.catalog_sha256 does not match "
                "the train split."
            )
        if not isinstance(
            raw_representation_statistics["contract_sha256"], str
        ) or not raw_representation_statistics["contract_sha256"]:
            raise ValueError(
                "action_representation_statistics.contract_sha256 must be a "
                "non-empty string."
            )
        if (
            raw_representation_statistics["contract_sha256"]
            != REALMAN_18D_ACTION_CONTRACT.sha256()
        ):
            raise ValueError(
                "action_representation_statistics.contract_sha256 does not "
                "match the exact RealMan 18-D contract."
            )
        if not isinstance(
            raw_representation_statistics["normalization"], str
        ) or not raw_representation_statistics["normalization"]:
            raise ValueError(
                "action_representation_statistics.normalization must be a "
                "non-empty string."
            )
        if raw_representation_statistics["normalization"] != Q01_Q99_UNCLIPPED:
            raise ValueError(
                "action_representation_statistics.normalization must be "
    ...
    return EpisodeSplitSelection(
        ...
        action_representation_statistics_path=representation_statistics_path,
        action_representation_statistics_sha256=representation_statistics_sha256,
