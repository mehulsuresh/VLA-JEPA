...
class TrainerUtils:
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # 2. update both the distributed sampler and the underlying dataset.
        # Accelerate's DataLoaderShard.set_epoch() normally updates only its
        # sampler.  With persistent workers that leaves worker-local dataset
        # copies stuck at epoch zero unless the dataset owns a shared epoch
        # value and the main-process dataset.set_epoch() is called explicitly.
    ...
    def _set_dataloader_epoch(dataloader, epoch: int) -> None:
        """Propagate an epoch through loader wrappers, samplers, and datasets."""
        visited: set[int] = set()
