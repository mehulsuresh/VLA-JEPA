...
LOG_ROOT="${ZERO2_FINAL_TUNE_LOG_ROOT:-/mnt/vla-jepa/logs/full_qwen_zero2_final_tune_$(date +%Y%m%d_%H%M%S)}"
STEPS="${ZERO2_FINAL_TUNE_STEPS:-12}"
MAX_SHARDS="${ZERO2_FINAL_TUNE_MAX_SHARDS:-8}"
BASE_PORT="${ZERO2_FINAL_TUNE_BASE_PORT:-29920}"
CASE_TIMEOUT_SECONDS="${ZERO2_FINAL_TUNE_CASE_TIMEOUT_SECONDS:-900}"
COMPILE_TIMEOUT_SECONDS="${ZERO2_FINAL_TUNE_COMPILE_TIMEOUT_SECONDS:-1200}"
PYTHON_BIN="${ZERO2_FINAL_TUNE_PYTHON_BIN:-${REPO_ROOT}/.venv/bin/python}"
mkdir -p "${LOG_ROOT}"
...
run_case() {
  ...
  run_id="z2final_${case_name}_${timestamp}"
  log_file="${LOG_ROOT}/${run_id}.log"
  global_batch=$((batch * 8))

  echo "=== ${case_name}: stage=2, batch=${batch}, global_batch=${global_batch}, workers=${workers}, prefetch=${prefetch}, grad_ckpt=${grad_ckpt}, compile=${compile_label}, save=${save_interval}, eval=${eval_interval} ==="
  (
    cd "${REPO_ROOT}" || exit 1
    ...
    timeout --signal=INT "${timeout_seconds}" "${LAUNCHER}" \
...
run_case "b24_w2p2_cache128_32_noio" 24 2 2 128 32 true 1000000 1000000 "$((BASE_PORT + 0))" "eager" "${CASE_TIMEOUT_SECONDS}"
run_case "b24_w1_gc_off_eager_noio" 24 1 2 128 32 false 1000000 1000000 "$((BASE_PORT + 1))" "eager" "${CASE_TIMEOUT_SECONDS}"
run_case "b32_w1_gc_on_eager_noio" 32 1 2 128 32 true 1000000 1000000 "$((BASE_PORT + 2))" "eager" "${CASE_TIMEOUT_SECONDS}"
run_case "b24_w1_gc_on_compile_action_vjpred" 24 1 2 128 32 true 1000000 1000000 "$((BASE_PORT + 3))" "action+vjpred" "${COMPILE_TIMEOUT_SECONDS}" \
...
run_case "b24_w1_gc_on_save_eval_step10" 24 1 2 128 32 true 10 10 "$((BASE_PORT + 4))" "eager" "${CASE_TIMEOUT_SECONDS}"

echo "ZeRO-2 final tuning summary: ${SUMMARY}"
