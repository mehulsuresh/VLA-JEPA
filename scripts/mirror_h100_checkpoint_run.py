...
OPTIONAL_CHECKPOINT_FILES = frozenset(
    {
        "selection_state.json",
        "resume_invocation.yaml",
    }
)
...
LEGACY_BASE_HELDOUT_REPORT_FIELDS = frozenset(
    {
    "schema_version",
    "purpose",
    "view",
...
BASE_HELDOUT_REPORT_FIELDS = frozenset(
    (LEGACY_BASE_HELDOUT_REPORT_FIELDS - {"frames_per_episode"})
    | {
...
CANONICAL_HELDOUT_REPORT_FIELDS = frozenset(
    {
        "schema_version",
        "purpose",
        "view",
...
CANONICAL_PROVENANCE_FIELDS = frozenset(
    {
        "dataset_name",
        "role",
        "manifest_sha256",
...
def _validate_data_stream_state(
    payload: Any,
    *,
    completed_steps: int,
) -> None:
    """Authenticate the nested exact-resume cursor in trainer_state.json."""
    ...
    if not isinstance(payload, Mapping) or set(payload) != required:
        raise MirrorError(
            "trainer_state.data_stream_state has incomplete or unexpected fields"
        )
    _require_schema_version(
        payload.get("schema_version"),
        label="trainer_state.data_stream_state.schema_version",
    )
    logical_epoch = _strict_int(
        payload.get("logical_epoch"),
        label="trainer_state.data_stream_state.logical_epoch",
    )
    offset = _strict_int(
        payload.get("local_batch_offset_in_epoch"),
        label="trainer_state.data_stream_state.local_batch_offset_in_epoch",
    )
    total = _strict_int(
        payload.get("total_local_batches_consumed"),
        label="trainer_state.data_stream_state.total_local_batches_consumed",
    )
    batches_per_epoch = _strict_int(
        payload.get("local_batches_per_epoch"),
        label="trainer_state.data_stream_state.local_batches_per_epoch",
        minimum=1,
    )
    optimizer_steps = _strict_int(
        payload.get("optimizer_steps_completed"),
        label="trainer_state.data_stream_state.optimizer_steps_completed",
    )
    grad_accum = _strict_int(
        payload.get("gradient_accumulation_steps"),
        label="trainer_state.data_stream_state.gradient_accumulation_steps",
        minimum=1,
    )
    if offset > batches_per_epoch:
        raise MirrorError(
            "trainer_state.data_stream_state offset exceeds its logical epoch"
        )
    if total != logical_epoch * batches_per_epoch + offset:
        raise MirrorError(
            "trainer_state.data_stream_state total/epoch/offset disagree"
        )
    if optimizer_steps != completed_steps:
        raise MirrorError(
            "trainer_state.data_stream_state optimizer step does not bind to "
            "trainer_state.completed_steps"
        )
    optimizer_boundary = (
        offset == batches_per_epoch or offset % grad_accum == 0
    )
    expected_optimizer_steps = (
        logical_epoch * math.ceil(batches_per_epoch / grad_accum)
        + (math.ceil(offset / grad_accum) if offset else 0)
    )
    if not optimizer_boundary or optimizer_steps != expected_optimizer_steps:
        raise MirrorError(
            "trainer_state.data_stream_state is not a valid completed "
            "optimizer boundary"
        )
    schedule_sha = payload.get("epoch_schedule_sha256")
    if not isinstance(schedule_sha, str) or not SHA256_RE.fullmatch(schedule_sha):
        raise MirrorError(
            "trainer_state.data_stream_state.epoch_schedule_sha256 must be "
            "lowercase SHA-256"
        )
    if payload.get("checkpoint_boundary") != "completed_optimizer_step":
        raise MirrorError(
            "trainer_state.data_stream_state.checkpoint_boundary is invalid"
        )
...
def _validate_checkpoint_resume_invocation(
    ...
    *,
    ...
    """Authenticate optional post-resume checkpoint lineage.

    The trainer embeds the exact validated resume invocation as a sidecar and
    binds it from ``trainer_state.json``.  Legacy and pre-lineage checkpoints
    remain valid, but a sidecar and its lineage record must always appear
    together so an arbitrary YAML file cannot enter a mirrored checkpoint.
    """

    sidecar = root / "resume_invocation.yaml"
    lineage = trainer_state.get("resume_invocation")
    if lineage is None:
        if sidecar.exists() or sidecar.is_symlink():
            raise MirrorError(
                "checkpoint resume_invocation.yaml is present without "
                "trainer_state.resume_invocation lineage"
            )
        return False
    ...
    if not isinstance(lineage, dict) or set(lineage) != expected_fields:
        raise MirrorError(
            "trainer_state.resume_invocation has incomplete or unexpected fields"
        )
    _require_schema_version(
        lineage.get("schema_version"),
        label="trainer_state.resume_invocation.schema_version",
    )
    if lineage.get("file") != "resume_invocation.yaml":
        raise MirrorError(
            "trainer_state.resume_invocation.file must be resume_invocation.yaml"
        )
    ...
    if not isinstance(expected_sha256, str) or not SHA256_RE.fullmatch(
        ...
        raise MirrorError(
            "trainer_state.resume_invocation.sha256 must be a lowercase SHA-256"
        )
    resumed_from_step = _strict_int(
        lineage.get("resumed_from_step"),
        label="trainer_state.resume_invocation.resumed_from_step",
    )
    if resumed_from_step > completed_steps:
        raise MirrorError(
            "trainer_state.resume_invocation.resumed_from_step is newer than "
            f"the containing checkpoint step {completed_steps}"
        )
    if type(lineage.get("runtime_override_required")) is not bool:
        raise MirrorError(
            "trainer_state.resume_invocation.runtime_override_required must be "
            "an exact boolean"
        )
    if sidecar.is_symlink() or not sidecar.is_file():
        raise MirrorError(
            "trainer_state.resume_invocation requires a regular "
            "resume_invocation.yaml sidecar"
        )
    if _sha256_file(sidecar) != expected_sha256:
        raise MirrorError(
            "checkpoint resume_invocation.yaml SHA-256 does not match "
            "trainer_state.resume_invocation"
        )
    return True
...
def validate_checkpoint_tree(
    ...
    *,
    ...
    has_resume_invocation = _validate_checkpoint_resume_invocation(
        root,
        trainer_state,
        completed_steps=completed_steps,
    )
    has_data_stream_state = "data_stream_state" in trainer_state
    if has_data_stream_state:
        _validate_data_stream_state(
            trainer_state["data_stream_state"],
            completed_steps=completed_steps,
        )
    ...
    if marker is None and (
        has_resume_invocation
        or has_data_stream_state
        or set(trainer_state) != {"completed_steps"}
    ):
        raise MirrorError(
    ...
    if marker is not None:
        ...
        if has_resume_invocation:
            expected_trainer_keys.add("resume_invocation")
        if has_data_stream_state:
            expected_trainer_keys.add("data_stream_state")
...
def validate_eval_artifact(
    ...
    *,
    ...
    expected_reports = run_evidence.sampling_reports()
    ...
    canonical_profile = run_evidence.focused_sampling_report is None
    allowed_metric_group_sets = (
        ({"unbiased"}, {"unbiased", "focused"})
        if canonical_profile
        else ({"unbiased", "focused"},)
    )
    if not isinstance(metric_groups, dict) or set(metric_groups) not in (
        allowed_metric_group_sets
    ):
    ...
    expected_selection_group = "unbiased" if canonical_profile else "focused"
    other_selection_group = (
        "focused" if expected_selection_group == "unbiased" else "unbiased"
    )
    if metric_name not in metric_groups[expected_selection_group] or (
        other_selection_group in metric_groups
        and metric_name in metric_groups[other_selection_group]
    ):
        raise MirrorError(
            f"configured {expected_selection_group} selection metric is placed "
            "incorrectly"
        )
    if canonical_profile and "focused" in metric_groups and metric_groups["focused"]:
        raise MirrorError("canonical eval cannot carry focused metrics")
...
def validate_baseline_eval_artifact(
    ...
    *,
    ...
    reports = payload.get("sampling_reports")
    if reports != run_evidence.sampling_reports():
    ...
    if canonical_profile and "focused" in metric_groups and metric_groups["focused"]:
        raise MirrorError("canonical baseline cannot carry focused metrics")
...
def _validate_split_provenance(
    ...
    *,
    ...
    for index, entry in enumerate(value):
        ...
        selected_episode_count = int(entry["selected_episode_count"])
        if selected_episode_count != int(entry["holdout_episode_count"]):
            raise MirrorError(
                f"{label} provenance[{index}] selected/holdout episode "
                "counts differ"
            )
        selected_total += selected_episode_count
    if selected_total != int(holdout_episode_count):
        raise MirrorError(
            f"{label} split provenance does not bind all "
            f"{holdout_episode_count} heldout episodes"
        )


def _is_canonical_heldout_report(report: Mapping[str, Any]) -> bool:
    return (
        report.get("purpose") == "canonical_manifest_heldout_training_health"
        or report.get("algorithm") == "exact_manifest_window_v1"
    )


def _validate_canonical_window_report_contract(
    report: Mapping[str, Any],
    *,
    expected_observations: int | None = None,
) -> None:
    """Validate the compact canonical-stream report without RealMan fields."""

    _require_schema_version(
        report.get("schema_version"), label="canonical window schema_version"
    )
    expected_fields = set(CANONICAL_HELDOUT_REPORT_FIELDS)
    allowed_shapes = {
        frozenset(expected_fields),
        frozenset(expected_fields | {"frames_per_episode"}),
    }
    if frozenset(report) not in allowed_shapes:
        expected_union = set().union(*allowed_shapes)
        raise MirrorError(
            "canonical heldout window report has incomplete/unexpected fields: "
            f"missing={sorted(expected_fields - set(report))}, "
            f"unexpected={sorted(set(report) - expected_union)}"
        )
    if (
        report.get("purpose") != "canonical_manifest_heldout_training_health"
        or report.get("view") != "unbiased"
        or report.get("algorithm") != "exact_manifest_window_v1"
    ):
        raise MirrorError("canonical heldout window identity is invalid")
    if report.get("production_valid") is not True or report.get(
        "checkpoint_selection_eligible"
    ) is not True:
        raise MirrorError("canonical heldout report is not production eligible")

    observations = _strict_count(
        report.get("observation_count"),
        label="canonical.observation_count",
        minimum=1,
    )
    if expected_observations is not None and observations != int(
        expected_observations
    ):
        raise MirrorError(
            "canonical heldout observation_count does not equal the resolved "
            "evaluation observation count"
        )
    holdout_episode_count = _strict_count(
        report.get("holdout_episode_count"),
        label="canonical.holdout_episode_count",
        minimum=1,
    )
    base_frames_per_episode = _strict_count(
        report.get("base_frames_per_episode"),
        label="canonical.base_frames_per_episode",
        minimum=1,
    )
    extra_window_episode_count = _strict_count(
        report.get("extra_window_episode_count"),
        label="canonical.extra_window_episode_count",
    )
    maximum_frames_per_episode = _strict_count(
        report.get("maximum_frames_per_episode"),
        label="canonical.maximum_frames_per_episode",
        minimum=1,
    )
    if (
        extra_window_episode_count >= holdout_episode_count
        ...
        raise MirrorError("canonical balanced episode/window allocation is invalid")
    if report.get("window_allocation_algorithm") not in {
        "balanced_digest_rank_v1",
        "uniform_per_episode_v1",
    }:
        raise MirrorError("canonical window allocation algorithm is invalid")
    extra_identities = report.get("extra_window_episode_identities")
    if not isinstance(extra_identities, list):
        raise MirrorError("canonical extra-window identities must be a list")
    normalized_extra_identities: list[tuple[str, str, str, str, int]] = []
    for index, identity in enumerate(extra_identities):
        if (
            ...
            raise MirrorError(
                f"canonical extra-window identity[{index}] is invalid"
            )
        normalized_extra_identities.append(
            (
                str(identity[0]),
                str(identity[1]),
                str(identity[2]),
    ...
    if (
        len(normalized_extra_identities) != extra_window_episode_count
        or len(set(normalized_extra_identities))
        != len(normalized_extra_identities)
    ):
        raise MirrorError("canonical extra-window identity cardinality is invalid")
    if extra_window_episode_count == 0:
        if report.get("frames_per_episode") != base_frames_per_episode:
            raise MirrorError("canonical uniform frames_per_episode alias is invalid")
        expected_observation_mode = (
            "one_window_per_heldout_episode"
            if base_frames_per_episode == 1
            else "balanced_deterministic_windows_per_heldout_episode"
        )
    else:
        if "frames_per_episode" in report:
            raise MirrorError(
                "canonical nonuniform allocation cannot claim frames_per_episode"
            )
        expected_observation_mode = (
            "balanced_deterministic_windows_per_heldout_episode"
        )
    if report.get("observation_mode") != expected_observation_mode:
        raise MirrorError("canonical heldout observation_mode is invalid")

    action_evaluable = _strict_count(
        report.get("action_evaluable_observation_count"),
        label="canonical.action_evaluable_observation_count",
    )
    if action_evaluable != observations or report.get(
        "zero_valid_action_episodes"
    ) != []:
        raise MirrorError("canonical report contains a zero-supervision observation")
    action_dim = _strict_count(
        report.get("action_dim"), label="canonical.action_dim", minimum=1
    )
    _strict_count(report.get("state_dim"), label="canonical.state_dim", minimum=1)
    action_horizon = _strict_count(
        report.get("action_horizon"),
        label="canonical.action_horizon",
        minimum=1,
    )
    valid_elements = _strict_count(
        report.get("valid_action_element_count"),
        label="canonical.valid_action_element_count",
        minimum=1,
    )
    possible_elements = _strict_count(
        report.get("possible_action_element_count"),
        label="canonical.possible_action_element_count",
        minimum=1,
    )
    if possible_elements != observations * action_horizon * action_dim:
        raise MirrorError("canonical possible-action element count is invalid")
    if valid_elements > possible_elements:
        raise MirrorError("canonical valid-action element count is impossible")
    valid_fraction = _finite_number(
        report.get("valid_action_fraction"),
        label="canonical.valid_action_fraction",
    )
    if not math.isclose(
        valid_fraction,
        valid_elements / possible_elements,
        rel_tol=0.0,
        abs_tol=1e-15,
    ):
        raise MirrorError("canonical valid-action fraction is inconsistent")
    channel_counts = report.get("valid_action_channel_counts")
    if not isinstance(channel_counts, list) or len(channel_counts) != action_dim:
        raise MirrorError("canonical valid-action channel counts are invalid")
    normalized_channel_counts = [
        _strict_count(
            value,
            label=f"canonical.valid_action_channel_counts[{index}]",
        )
        for index, value in enumerate(channel_counts)
    ]
    if (
        sum(normalized_channel_counts) != valid_elements
        or any(value > observations * action_horizon for value in normalized_channel_counts)
    ):
        raise MirrorError("canonical valid-action channel counts are inconsistent")
    ...
    for field in (
        ...
        if not isinstance(report.get(field), str) or not report[field]:
            raise MirrorError(f"canonical.{field} is invalid")
    if report.get("normalization_statistics_scope") != "train_episodes_only":
        raise MirrorError("canonical normalization statistics include holdout data")
    for field in (
        "window_selection_sha256",
        "canonical_eval_manifest_sha256",
        "source_manifest_sha256",
        ...
        if not isinstance(value, str) or SHA256_RE.fullmatch(value) is None:
            raise MirrorError(f"canonical.{field} is invalid")
    for field in ("train_holdout_disjoint", "normalization_excludes_holdout"):
        if report.get(field) is not True:
            raise MirrorError(f"canonical.{field} must be true")
    for field in ("control_metadata_required", "subtask_labels_required"):
        if report.get(field) is not False:
            raise MirrorError(f"canonical.{field} must be false")
    if report.get("metric_horizons") != [10, 50] or report.get(
        "metric_groups"
    ) != ["all_action", "arm", "hand"]:
        raise MirrorError("canonical metric coverage contract is invalid")
    for field in (
        "subtask_observation_counts",
        "subtask_evaluable_observation_counts",
        "subtask_action_timestep_counts_by_horizon",
        ...
        if report.get(field) != {}:
            raise MirrorError(f"canonical.{field} must be empty")

    provenance = report.get("episode_split_provenance")
    if not isinstance(provenance, list) or len(provenance) != 1:
        raise MirrorError("canonical split provenance shape is invalid")
    entry = provenance[0]
    if not isinstance(entry, dict) or set(entry) != CANONICAL_PROVENANCE_FIELDS:
        raise MirrorError("canonical split provenance fields are invalid")
    if (
        entry.get("dataset_name") != "canonical_stream"
        or str(entry.get("role", "")).lower() != "holdout"
        or _strict_count(
            entry.get("selected_episode_count"),
            label="canonical provenance selected_episode_count",
            minimum=1,
        )
        != holdout_episode_count
    ):
        raise MirrorError("canonical split provenance identity is invalid")
    for field in CANONICAL_PROVENANCE_FIELDS - {
        ...
        if not isinstance(value, str) or SHA256_RE.fullmatch(value) is None:
            raise MirrorError(f"canonical provenance {field} is invalid")
    if entry["manifest_sha256"] != report["canonical_eval_manifest_sha256"]:
        raise MirrorError("canonical manifest digest contradicts split provenance")
...
def _validate_window_report_contracts(
    ...
    *,
    ...
    allocation_contracts: dict[str, dict[str, Any]] = {}
    for label, report, expected_view, expected_purpose in (
        ...
        modern_allocation = "holdout_episode_count" in report
        expected_fields = set(
            BASE_HELDOUT_REPORT_FIELDS
            if modern_allocation
            else LEGACY_BASE_HELDOUT_REPORT_FIELDS
        )
        if label == "focused":
            expected_fields.update(FOCUSED_HELDOUT_REPORT_FIELDS)
        allowed_shapes = {frozenset(expected_fields)}
        if modern_allocation:
            allowed_shapes.add(frozenset(expected_fields | {"frames_per_episode"}))
        if frozenset(report) not in allowed_shapes:
            expected_union = set().union(*allowed_shapes)
            raise MirrorError(
                f"{label} heldout window report has incomplete/unexpected fields: "
                f"missing={sorted(set(expected_fields) - set(report))}, "
                f"unexpected={sorted(set(report) - expected_union)}"
            )
        allowed_purposes = {expected_purpose}
        if label == "unbiased":
            allowed_purposes.add(
                "deterministic_windows_per_manifest_holdout_episode_checkpoint_eval"
            )
        if (
            report.get("view") != expected_view
            or report.get("purpose") not in allowed_purposes
        ):
        ...
        if modern_allocation:
            if (
                report.get("control_metadata_required") is not True
                or report.get("subtask_labels_required") is not True
            ):
                raise MirrorError(
                    f"{label} RealMan heldout metadata requirements are invalid"
                )
            holdout_episode_count = _strict_count(
                report.get("holdout_episode_count"),
                label=f"{label}.holdout_episode_count",
                minimum=1,
            )
            base_frames_per_episode = _strict_count(
                report.get("base_frames_per_episode"),
                label=f"{label}.base_frames_per_episode",
                minimum=1,
            )
            extra_window_episode_count = _strict_count(
                report.get("extra_window_episode_count"),
                label=f"{label}.extra_window_episode_count",
            )
            maximum_frames_per_episode = _strict_count(
                report.get("maximum_frames_per_episode"),
                label=f"{label}.maximum_frames_per_episode",
                minimum=1,
            )
            if (
                extra_window_episode_count >= holdout_episode_count
                ...
                raise MirrorError(
                    f"{label} balanced episode/window allocation is invalid"
                )
            window_allocation_algorithm = report.get(
                "window_allocation_algorithm"
            )
            if window_allocation_algorithm not in {
                "balanced_digest_rank_v1",
                "uniform_per_episode_v1",
                "legacy_uniform_per_episode_v1",
            }:
                raise MirrorError(
                    f"{label} window allocation algorithm is invalid"
                )
            raw_extra_identities = report.get(
                "extra_window_episode_identities"
            )
            if not isinstance(raw_extra_identities, list):
                raise MirrorError(
                    f"{label} extra-window episode identities must be a list"
                )
            extra_window_identities: list[tuple[str, int]] = []
            for index, identity in enumerate(raw_extra_identities):
                if (
                    ...
                    raise MirrorError(
                        f"{label} extra-window identity[{index}] is invalid"
                    )
                extra_window_identities.append(
                    (
            ...
            if (
                len(extra_window_identities)
                != extra_window_episode_count
                or len(set(extra_window_identities))
                != len(extra_window_identities)
            ):
                raise MirrorError(
                    f"{label} extra-window identity cardinality is invalid"
                )
            frames_per_episode = report.get("frames_per_episode")
            if extra_window_episode_count == 0:
                if frames_per_episode != base_frames_per_episode:
                    raise MirrorError(
                        f"{label} uniform frames_per_episode alias is invalid"
                    )
            elif "frames_per_episode" in report:
                raise MirrorError(
                    f"{label} nonuniform allocation cannot claim a scalar "
                    "frames_per_episode"
                )
        else:
            frames_per_episode = _strict_count(
                report.get("frames_per_episode"),
                label=f"{label}.frames_per_episode",
                minimum=1,
            )
            base_frames_per_episode = frames_per_episode
            extra_window_episode_count = 0
            maximum_frames_per_episode = frames_per_episode
            holdout_episode_count = observations // frames_per_episode
            extra_window_identities = []
            if holdout_episode_count * frames_per_episode != observations:
                raise MirrorError(
                    f"{label} legacy uniform allocation is invalid"
                )
            window_allocation_algorithm = "legacy_uniform_per_episode_v1"
        allocation_contracts[label] = {
            "modern": bool(modern_allocation),
            ...
            "window_allocation_algorithm": str(window_allocation_algorithm),
            "extra_window_episode_identities": tuple(extra_window_identities),
            "frames_per_episode": (
                None
                if extra_window_episode_count
                else int(base_frames_per_episode)
            ),
        }
        expected_unbiased_purpose = (
            "one_window_per_manifest_holdout_episode_checkpoint_eval"
            if (
                base_frames_per_episode == 1
                and extra_window_episode_count == 0
            )
            else (
                "deterministic_windows_per_manifest_holdout_episode_"
                "checkpoint_eval"
            )
        )
        if label == "unbiased" and report.get("purpose") != expected_unbiased_purpose:
            raise MirrorError(
                "unbiased heldout purpose does not match frames_per_episode"
            )
        ...
        seen_windows: set[tuple[int, str, int, int]] = set()
        for window_index, raw_window in enumerate(windows):
            ...
            identity = (dataset_index, dataset_name, episode_id, base_index)
            if identity in seen_windows:
                raise MirrorError(f"{label} contains duplicate heldout window {identity!r}")
        ...
        per_dataset_episodes: dict[int, set[tuple[str, int]]] = {}
        for window in normalized:
            ...
            per_dataset_episodes.setdefault(dataset_index, set()).add(
                (str(window["dataset_name"]), int(window["episode_id"]))
            )
        per_episode: dict[tuple[int, str, int], int] = {}
        for window in normalized:
            key = (
                int(window["dataset_index"]),
                str(window["dataset_name"]),
                int(window["episode_id"]),
            )
            per_episode[key] = per_episode.get(key, 0) + 1
        observed_episode_identities = {
            (dataset_name, episode_id)
            for _, dataset_name, episode_id in per_episode
        }
        extra_identity_set = set(extra_window_identities)
        if not per_episode or extra_identity_set - observed_episode_identities:
            raise MirrorError(
                f"{label} extra-window identities are not bound to heldout episodes"
            )
        invalid_episode_counts = {
            (dataset_name, episode_id): count
            for (_, dataset_name, episode_id), count in per_episode.items()
            if count
            != base_frames_per_episode
            + int((dataset_name, episode_id) in extra_identity_set)
        }
        if invalid_episode_counts:
            raise MirrorError(
                f"{label} per-episode window counts do not match its q/r "
                f"allocation: {invalid_episode_counts!r}"
            )
        for index, entry in enumerate(provenance):
            selected_episode_count = int(entry["selected_episode_count"])
            dataset_episodes = per_dataset_episodes.get(index, set())
            if len(dataset_episodes) != selected_episode_count:
                raise MirrorError(
                    f"{label} per-dataset episode counts do not match provenance"
                )
            dataset_name = str(entry["dataset_name"])
            dataset_extra_count = sum(
                identity[0] == dataset_name for identity in extra_identity_set
            )
            expected_dataset_windows = (
                selected_episode_count * base_frames_per_episode
                + dataset_extra_count
            )
            if per_dataset.get(index, 0) != expected_dataset_windows:
                raise MirrorError(
                    f"{label} per-dataset window counts do not match provenance"
                )
        if len(per_episode) != holdout_episode_count:
            raise MirrorError(
                f"{label} unique heldout episode count does not match allocation"
            )
    shared_fields = {
        "observation_mode",
        "evaluation_video_offsets",
    ...
    if allocation_contracts["unbiased"] != allocation_contracts["focused"]:
        raise MirrorError(
            "focused and unbiased episode/window allocation contracts differ"
        )
    ...
    unbiased_allocation = allocation_contracts["unbiased"]
    if unbiased_allocation["modern"]:
        unbiased_contract = {
            "algorithm": unbiased.get("algorithm"),
            "observation_mode": unbiased.get("observation_mode"),
            "evaluation_video_offsets": unbiased.get(
                "evaluation_video_offsets"
            ),
            "action_offset_range_inclusive": unbiased.get(
                "action_offset_range_inclusive"
            ),
            "observation_count": unbiased_allocation["observation_count"],
            "holdout_episode_count": unbiased_allocation[
                "holdout_episode_count"
            ],
            "window_allocation": {
                "algorithm": unbiased_allocation[
                    "window_allocation_algorithm"
                ],
                "base_frames_per_episode": unbiased_allocation[
                    "base_frames_per_episode"
                ],
                "extra_window_episode_count": unbiased_allocation[
                    "extra_window_episode_count"
                ],
                "maximum_frames_per_episode": unbiased_allocation[
                    "maximum_frames_per_episode"
                ],
                "extra_window_episode_identities": [
                    {
                    ...
                    for dataset_name, episode_id in unbiased_allocation[
                        "extra_window_episode_identities"
                    ]
                ],
            },
        ...
        unbiased_contract = {
            ...
            "frames_per_episode": unbiased_allocation[
                "frames_per_episode"
            ],
            "seed_sha256": unbiased.get("seed_sha256"),
            "windows": unbiased.get("windows"),
...
class RunEvidenceIdentity:
    ...
    focused_sampling_report: Mapping[str, Any] | None

    def sampling_reports(self) -> dict[str, dict[str, Any]]:
        reports = {"unbiased": dict(self.unbiased_sampling_report)}
        if self.focused_sampling_report is not None:
            reports["focused"] = dict(self.focused_sampling_report)
        return reports
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "run_id": self.run_id,
            "seed": self.seed,
            "sampling_reports": self.sampling_reports(),
        }
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "RunEvidenceIdentity":
        ...
        reports = payload.get("sampling_reports")
        if not isinstance(reports, dict) or set(reports) not in (
            {"unbiased"},
            {"unbiased", "focused"},
        ):
        ...
        if "focused" in reports:
            if _is_canonical_heldout_report(reports["unbiased"]):
                raise MirrorError("canonical run evidence cannot carry a focused report")
            _validate_window_report_contracts(
                reports["unbiased"], reports["focused"]
            )
        else:
            _validate_canonical_window_report_contract(reports["unbiased"])
        ...
        return cls(
            ...
            focused_sampling_report=(
                None
                if "focused" not in reports
                else dict(reports["focused"])
            ),
        )
...
def validate_run_evidence_identity(
    ...
    focused_path = root / "heldout_focused_eval_windows.json"
    focused_raw = (
        _load_json_object(
            focused_path,
    ...
    resolved_eval_count = resolved.get(
    ...
    expected_observations = _strict_count(
        resolved_eval_count,
        label=(
            "resolved evaluation_observation_count"
            if "evaluation_observation_count" in resolved
            else "legacy resolved effective_global_batch_size"
        ),
        minimum=1,
    )
    canonical_report = _is_canonical_heldout_report(unbiased_raw)
    if canonical_report:
        if focused_raw is not None:
            raise MirrorError(
                "canonical run evidence must not carry a RealMan focused report"
            )
        _validate_canonical_window_report_contract(
            unbiased_raw,
        ...
        if focused_raw is None:
            raise MirrorError(
                "RealMan run evidence requires heldout_focused_eval_windows.json"
            )
        _validate_window_report_contracts(
            unbiased_raw,
            focused_raw,
        ...
        _canonical_json_bytes(
            {
                ...
                "seed": evidence.seed,
                "sampling_reports": evidence.sampling_reports(),
