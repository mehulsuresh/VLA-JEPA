...
STARVLA_CONFIG_IS_AUTHORITATIVE="${STARVLA_CONFIG_IS_AUTHORITATIVE:-0}"
if [[ "${STARVLA_CONFIG_IS_AUTHORITATIVE}" != "0" \
  && "${STARVLA_CONFIG_IS_AUTHORITATIVE}" != "1" ]]; then
  echo "STARVLA_CONFIG_IS_AUTHORITATIVE must be 0 or 1; got ${STARVLA_CONFIG_IS_AUTHORITATIVE}" >&2
  exit 2
fi
...
if [[ "${STARVLA_USE_DEEPSPEED}" == "1" ]]; then
  ...
  if [[ "${STARVLA_CONFIG_IS_AUTHORITATIVE}" != "1" ]]; then
    STARVLA_EXTRA_TRAIN_ARGS+=(
...
if [[ "${STARVLA_CONFIG_IS_AUTHORITATIVE}" == "1" ]]; then
  # Runtime checkpoint identity is not a training hyperparameter. Everything
  # else must be reviewed in CONFIG_YAML and is rejected here.
  authoritative_args=("$@")
  authoritative_index=0
  while (( authoritative_index < ${#authoritative_args[@]} )); do
    authoritative_arg="${authoritative_args[authoritative_index]}"
    if [[ "${authoritative_arg}" != --* ]]; then
      echo "Authoritative config launcher refuses orphaned argument: ${authoritative_arg}" >&2
      exit 2
    fi
    authoritative_option="${authoritative_arg%%=*}"
    case "${authoritative_option}" in
      --trainer.is_resume | --resume_from_checkpoint | --trainer.resume_from_checkpoint)
        ;;
      *)
        echo "Authoritative config launcher refuses training override: ${authoritative_option}" >&2
        exit 2
        ;;
    esac
    if [[ "${authoritative_arg}" == *=* ]]; then
      authoritative_index=$((authoritative_index + 1))
    else
      if (( authoritative_index + 1 >= ${#authoritative_args[@]} )) \
        || [[ "${authoritative_args[authoritative_index + 1]}" == --* ]]; then
        echo "Authoritative config runtime argument requires a value: ${authoritative_option}" >&2
        exit 2
      fi
      authoritative_index=$((authoritative_index + 2))
    fi
  done
fi
