...
DOCKER_AUTO_REMOVE="${DOCKER_AUTO_REMOVE:-1}"
DOCKER_DETACH="${DOCKER_DETACH:-0}"
if [[ "${DOCKER_AUTO_REMOVE}" != "0" && "${DOCKER_AUTO_REMOVE}" != "1" ]]; then
  echo "DOCKER_AUTO_REMOVE must be 0 or 1" >&2
  exit 1
fi
if [[ "${DOCKER_DETACH}" != "0" && "${DOCKER_DETACH}" != "1" ]]; then
  echo "DOCKER_DETACH must be 0 or 1" >&2
  exit 1
fi
...
if [[ -n "${DOCKER_USER:-}" ]]; then
  if [[ ! "${DOCKER_USER}" =~ ^[0-9]+(:[0-9]+)?$ ]]; then
    echo "Invalid DOCKER_USER=${DOCKER_USER}; expected UID or UID:GID" >&2
    exit 1
  fi
  DOCKER_ARGS+=(--user "${DOCKER_USER}")
fi
if [[ -n "${DOCKER_HOME:-}" ]]; then
  DOCKER_ARGS+=(-e "HOME=${DOCKER_HOME}")
fi
...
if [[ "${MOUNT_GCLOUD:-auto}" != "0" && -d "${GCLOUD_SDK_ROOT}" && -n "${GCLOUD_CONFIG_DIR:-}" && -d "${GCLOUD_CONFIG_DIR}" ]]; then
  GCLOUD_CONTAINER_CONFIG="${GCLOUD_CONTAINER_CONFIG:-${DOCKER_HOME:-/root}/.config/gcloud}"
  if [[ "${GCLOUD_CONTAINER_CONFIG}" != /* || "${GCLOUD_CONTAINER_CONFIG}" == *:* ]]; then
    echo "GCLOUD_CONTAINER_CONFIG must be an absolute container path without ':'" >&2
    exit 1
  fi
  DOCKER_ARGS+=(
    -e "CLOUDSDK_CONFIG=${GCLOUD_CONTAINER_CONFIG}"
  ...
  add_docker_mount "${GCLOUD_CONFIG_DIR}" "${GCLOUD_CONTAINER_CONFIG}"
