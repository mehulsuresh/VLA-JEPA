...
# This historical bounded pilot predates config-authoritative production
# launchers and deliberately builds a temporary profile below. It must never be
# used for a new full production run.
export STARVLA_CONFIG_IS_AUTHORITATIVE=0
...
# This archived pilot intentionally uses the generic legacy-capable launcher.
# Never route it through either config-authoritative production profile.
