...
LOG_ROOT="${TARGET_BENCHMARK_LOG_ROOT:-/mnt/vla-jepa/logs/full_qwen_targeted_benchmarks}"
STEPS="${TARGET_BENCHMARK_STEPS:-12}"
MAX_SHARDS="${TARGET_BENCHMARK_MAX_SHARDS:-8}"
BASE_PORT="${TARGET_BENCHMARK_BASE_PORT:-29710}"
CASE_TIMEOUT_SECONDS="${TARGET_BENCHMARK_CASE_TIMEOUT_SECONDS:-1500}"
PYTHON_BIN="${TARGET_BENCHMARK_PYTHON_BIN:-${REPO_ROOT}/.venv/bin/python}"
mkdir -p "${LOG_ROOT}"
...
summarize_case() {
  ...
  local batch="$2"
  local exit_code="$3"
  "${PYTHON_BIN}" - "$log_file" "$batch" "$exit_code" <<'PY'
import math
import re
import sys
...
run_case() {
  ...
  run_id="target_${case_name}_${timestamp}"
  ...
  metrics="$(summarize_case "${log_file}" "${batch}" "${exit_code}")"
...
run_case "z3_b16_w1_gc_on_eager_noio" 3 16 1 2 128 32 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b24_w1_gc_on_eager_noio" 3 24 1 2 128 32 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b32_w1_gc_on_eager_noio" 3 32 1 2 128 32 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b16_w1_gc_off_eager_noio" 3 16 1 2 128 32 false 1000000 1000000 "$(next_port)" "eager"
run_case "z2_b16_w1_gc_on_eager_noio" 2 16 1 2 128 32 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b16_w2p2_gc_on_cache128_32_noio" 3 16 2 2 128 32 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b16_w4p4_gc_on_cache256_64_noio" 3 16 4 4 256 64 true 1000000 1000000 "$(next_port)" "eager"
run_case "z3_b16_w1_gc_on_compile_action_vjpred" 3 16 1 2 128 32 true 1000000 1000000 "$(next_port)" "action+vjpred" \
...
run_case "z3_b16_w1_gc_on_save_eval_step10" 3 16 1 2 128 32 true 10 10 "$(next_port)" "eager"

echo "Targeted benchmark summary: ${SUMMARY}"
