...
- Treat each full Magna production YAML as the complete training contract.
  Production launchers must not translate environment variables or command-line
  dotlists into data, model, optimization, schedule, evaluation, or checkpoint
  settings. The only permitted training arguments at launch time identify an
  explicitly validated resume checkpoint; infrastructure values such as the run
  ID, image, process topology, port, caches, and service paths remain runtime
  inputs. A bounded smoke or lifecycle run requires its own committed YAML.
...
| Historical 18D-action/19D-state config, 8 GPUs, batch 12/rank | Passed two optimizer steps, but is incompatible with the OpenPI-compatible 18D-state contract and must not be resumed | `/mnt/vla-jepa/logs/magna_a100x8_18d_smoke_f2aafad_20260711_0910.result.txt` |
...
dimensions 16:19 (base velocity) and 21 (lift height), and omits state index 18
(lift), producing 18D action and 18D state tensors. The measured lift height is
kept outside the model by the robot client. The 18D model target uses
`joint_delta_gripper_absolute`: each arm/head position in the full 50-step
chunk is expressed relative to one selected 18D state at the chunk
anchor, while action indices 7 and 15 remain absolute gripper commands. Delta
conversion occurs in raw units before unclipped OpenPI q01/q99 normalization;
deployment must unnormalize first and then add the same current-state anchor
back only on joint channels. Base commands are expanded as zero and lift is
expanded from the current measurement. The main
...
ssh "$HOST" '
  ...
  cd /mnt/vla-jepa/src/VLA-JEPA
  {
    ...
    sha256sum \
      ...
      scripts/vlajepa_robot_ft_lerobot_magna_interventions_a100x8_qwen35_2b_full_moge_vitb_vjepa_large.sh \
      scripts/vlajepa_robot_ft_libero_plus_a100x8_qwen3_full_moge_vitb_vjepa_large.sh \
      ...
      deployment/realman/eval_manifests/artifacts/magna_internal_holdout_global_batch96_v1_openpi_realman18_stats.json \
...
The full A100 and H100 launchers pin their reviewed YAML and run in
config-authoritative mode. They reject CLI training overrides and ignore stale
environment variables such as `MAX_TRAIN_STEPS`, `SAVE_INTERVAL`,
`EVAL_INTERVAL`, `DATA_ROOT_DIR`, and `PER_DEVICE_BATCH_SIZE`. Keep every
static training value in the YAML. Runtime launch state is limited to run and
service identity, process topology, caches, and an optional validated resume
checkpoint.
...
2. Generate the exact OpenPI statistics and run the deterministic loader parity
   gate before any training smoke:

   ```bash
   python scripts/compute_openpi_realman_stats.py \
     --manifest deployment/realman/eval_manifests/magna_internal_holdout_global_batch96_v1.json \
     --update-manifest
   /home/mehul/work/reward_model_small/pi0/.venv/bin/python \
     scripts/check_openpi_realman_parity.py \
     --manifest deployment/realman/eval_manifests/magna_internal_holdout_global_batch96_v1.json \
     --output-dir local_eval_reports/openpi_parity_a100
   ```

   The parity report must say `passed: true`; any failure writes channel-level
   differences and `failure_outputs.npz`.
3. A direct dataset sample: verify action `[50, 18]`, state `[1, 18]`, three
   video views, an action mask, finite tensors, and a resolved prompt.
4. A two-step single-GPU forward/backward smoke from upstream initialization.
   Never resume an absolute-action or 19D-state checkpoint.
5. A short 8-GPU production-shape smoke long enough to pass model/cache warmup.
6. Verify no OOM, NaN/Inf, worker respawn loop, PyAV thread growth, NCCL error,
   all-zero-mask spike, or rank divergence.
7. Measure steady step time and peak allocated/reserved memory on every GPU.
...
Use a unique smoke run ID. Put the bounded budget, checkpoint/eval cadence,
data path, and `save_final_model: false` in a separate reviewed and committed
smoke YAML; never inject them through the full production launcher. The generic
launcher may execute that dedicated config in authoritative mode without any
dotlist arguments:
...
ssh "$HOST" '
  ...
  SMOKE_CONFIG=${PWD}/scripts/config/vlajepa_robot_ft_lerobot_magna_a100x8_smoke.yaml
...
- 18D no-base/no-lift action mapping, 18D state input, anchor-relative
  arm/head joints, and absolute gripper semantics;
...
The following block records the archived 2026-07-14 clean-pilot procedure. It
uses the explicitly legacy pilot launcher and must not be copied for a new
production or smoke run. New runs use one of the config-authoritative profile
launchers (or the generic launcher with a dedicated committed YAML).

```bash
ssh "$HOST" '
  set -e
  REPO=/mnt/vla-jepa/src/VLA-JEPA
  # Archived bounded corrected-data learning gate; do not reuse for a new run.
...
| 2026-07-21 | Action target changed from absolute positions to pi-style state-relative joints with absolute grippers | Bound the mixed representation in canonical and LeRobot configs, normalization, checkpoint metadata, offline evaluation, and deployment inversion. All earlier checkpoints, throughput smokes, and action-quality baselines are schema-incompatible and must not seed or qualify a new run. |
