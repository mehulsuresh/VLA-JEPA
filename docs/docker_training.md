...
> **Starting the production Magna H100x8 run?** Use the short, config-only
> [`H100x8 human quickstart`](h100_training_quickstart.md). It builds, validates,
> starts, resumes, and tails the run without the lower-level commands below.
...
The examples below are low-level development examples. They are not the
production Magna H100x8 launch path and may use command-line test overrides.
