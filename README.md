...
For the production Magna H100x8 run, use the human, config-only
[`H100x8 training quickstart`](./docs/h100_training_quickstart.md). It reduces
setup and launch to `setup`, `plan`, `check`, and `start`; all run settings live
in the reviewed YAML and command-line hyperparameter overrides are rejected.
The lower-level Docker runtime is documented in
[`docs/docker_training.md`](./docs/docker_training.md). Agents preparing or
handing off the older Magna A100x8 workflow must start with the living
...
- [`docs/canonical_checkpoint_eval.md`](./docs/canonical_checkpoint_eval.md) — compact, leakage-safe canonical/streaming checkpoint evaluation.
