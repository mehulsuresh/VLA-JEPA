...
def test_extra_milestone_runs_checkpoint_and_eval_once_at_optimizer_boundary():
    trainer, events = _loop_trainer(
        sync_sequence=[False, True, False, True, False, True],
        max_train_steps=3,
        eval_interval=99,
        save_interval=99,
        checkpoint_eval_milestone_steps=[2],
    )

    trainer.train()

    assert [event for event in events if event[0] == "save"] == [("save", 2)]
    assert [event for event in events if event[0] == "eval"] == [("eval", 2)]
    assert events.index(("save", 2)) < events.index(("eval", 2))


def test_extra_milestone_and_periodic_boundary_do_not_double_run():
    trainer, events = _loop_trainer(
        sync_sequence=[True, True],
        max_train_steps=2,
        eval_interval=2,
        save_interval=2,
        checkpoint_eval_milestone_steps=[2],
...
def test_milestone_only_mode_disables_inherited_periodic_boundaries():
    trainer, events = _loop_trainer(
        sync_sequence=[True, True, True, True],
        max_train_steps=4,
        eval_interval=2,
        save_interval=2,
        checkpoint_eval_milestone_steps=[3],
        checkpoint_eval_milestones_only=True,
    )

    trainer.train()

    assert [event for event in events if event[0] == "save"] == [("save", 3)]
    assert [event for event in events if event[0] == "eval"] == [("eval", 3)]


def test_resume_does_not_repeat_completed_milestone():
    trainer, events = _loop_trainer(
        sync_sequence=[True, True, True],
        completed_steps=2,
        max_train_steps=5,
        eval_interval=99,
        save_interval=99,
        checkpoint_eval_milestone_steps=[2, 4],
    )

    trainer.train()

    assert [event for event in events if event[0] == "save"] == [("save", 4)]
    assert [event for event in events if event[0] == "eval"] == [("eval", 4)]
