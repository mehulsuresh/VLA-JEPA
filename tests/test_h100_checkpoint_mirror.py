...
CANONICAL_METRIC = "heldout_eval_normalized_action_mae"
MODE = "min"
RUN_ID = "robot_ft_test_20260714"


def _selection_payload(best_step=None, value=None, *, metric=METRIC, mode=MODE):
    return {
        "schema_version": 1,
...
def _add_resume_invocation(
    checkpoint: Path,
    *,
    resumed_from_step: int,
    runtime_override_required: bool = True,
) -> bytes:
    payload = (
        "trainer:\n"
        f"  resume_from_checkpoint: /run/checkpoints/steps_{resumed_from_step}\n"
        "datasets:\n"
        "  vla_data:\n"
        "    num_workers: 4\n"
        "    multiprocessing_context: forkserver\n"
    ).encode("utf-8")
    (checkpoint / "resume_invocation.yaml").write_bytes(payload)
    ...
    trainer_state["resume_invocation"] = {
        ...
        "runtime_override_required": runtime_override_required,
    }
...
def _add_data_stream_state(checkpoint: Path) -> dict:
    trainer_state_path = checkpoint / "trainer_state.json"
    trainer_state = json.loads(trainer_state_path.read_text(encoding="utf-8"))
    ...
    payload = {
        "schema_version": 1,
        "logical_epoch": completed_steps // batches_per_epoch,
        "local_batch_offset_in_epoch": completed_steps % batches_per_epoch,
        "total_local_batches_consumed": completed_steps,
        "local_batches_per_epoch": batches_per_epoch,
        "optimizer_steps_completed": completed_steps,
        "gradient_accumulation_steps": 1,
        "epoch_schedule_sha256": "a" * 64,
        "checkpoint_boundary": "completed_optimizer_step",
    }
    trainer_state["data_stream_state"] = payload
    trainer_state_path.write_text(
        json.dumps(trainer_state),
        encoding="utf-8",
    )
...
def _reseal_report_digests(unbiased, focused):
    if "holdout_episode_count" in unbiased:
        unbiased_contract = {
            "algorithm": unbiased["algorithm"],
            "observation_mode": unbiased["observation_mode"],
            "evaluation_video_offsets": unbiased["evaluation_video_offsets"],
            "action_offset_range_inclusive": unbiased[
                "action_offset_range_inclusive"
            ],
            "observation_count": unbiased["observation_count"],
            "holdout_episode_count": unbiased["holdout_episode_count"],
            "window_allocation": {
                "algorithm": unbiased["window_allocation_algorithm"],
                "base_frames_per_episode": unbiased[
                    "base_frames_per_episode"
                ],
                "extra_window_episode_count": unbiased[
                    "extra_window_episode_count"
                ],
                "maximum_frames_per_episode": unbiased[
                    "maximum_frames_per_episode"
                ],
                "extra_window_episode_identities": unbiased[
...
def _modern_production_reports(window_counts):
    counts = tuple(int(value) for value in window_counts)
    if not counts or min(counts) <= 0 or max(counts) - min(counts) > 1:
        raise ValueError("window_counts must encode one balanced q/r allocation")
    unbiased, focused = _production_reports(batch_size=len(counts))

    def expand(report):
        original = report["windows"]
        expanded = []
        for episode_id, count in enumerate(counts):
            for slot in range(count):
                window = copy.deepcopy(original[episode_id])
                window["base_index"] += slot
                expanded.append(window)
        report["windows"] = expanded
    ...
    extra_identities = [
        {"dataset_name": "fixture", "episode_id": episode_id}
        for episode_id in extra_episode_ids
    ]
    horizons = (1, 5, 10, 20, 50)
    for report in (unbiased, focused):
        report.update(
            {
                "control_metadata_required": True,
                "subtask_labels_required": True,
                "holdout_episode_count": len(counts),
                "base_frames_per_episode": base,
                "extra_window_episode_count": len(extra_episode_ids),
                "maximum_frames_per_episode": max(counts),
                "window_allocation_algorithm": "balanced_digest_rank_v1",
                "extra_window_episode_identities": extra_identities,
                "observation_count": observations,
                "action_evaluable_observation_count": observations,
                "valid_action_timestep_count": observations * 50,
                "valid_action_element_count": observations * 900,
                "subtask_observation_counts": {"2": observations},
                "subtask_evaluable_observation_counts": {"2": observations},
                "subtask_action_timestep_counts_by_horizon": {
                    str(horizon): {"2": observations * horizon}
                    for horizon in horizons
                },
                "subtask_valid_action_element_counts_by_horizon": {
                    str(horizon): {"2": observations * horizon * 18}
                    for horizon in horizons
                },
            }
        )
        if extra_episode_ids:
            report.pop("frames_per_episode")
        else:
            report["frames_per_episode"] = base
    unbiased["purpose"] = (
        "one_window_per_manifest_holdout_episode_checkpoint_eval"
        if base == 1 and not extra_episode_ids
        else "deterministic_windows_per_manifest_holdout_episode_checkpoint_eval"
    )
    focused.update(
        {
            "open_to_close_transition_count_h10": observations,
            "close_to_open_transition_count_h10": observations,
            "open_to_close_transition_window_count_h10": observations,
            "close_to_open_transition_window_count_h10": observations,
            "arm_movement_element_count_h10": observations * 10,
            "arm_movement_hold_abs_sum_h10": float(observations),
        }
    )
...
def _canonical_report(
    *,
    observation_count: int = 128,
    holdout_episode_count: int = 32,
) -> dict:
    base, remainder = divmod(observation_count, holdout_episode_count)
    assert base > 0 and 0 <= remainder < holdout_episode_count
    action_dim = 18
    action_horizon = 50
    possible_elements = observation_count * action_horizon * action_dim
    manifest_sha = "8" * 64
    report = {
        "schema_version": 1,
        ...
        "observation_mode": (
            ...
            if base == 1 and remainder == 0
            else "balanced_deterministic_windows_per_heldout_episode"
        ),
        "holdout_episode_count": holdout_episode_count,
        "base_frames_per_episode": base,
        "extra_window_episode_count": remainder,
        "maximum_frames_per_episode": base + int(remainder > 0),
        "window_allocation_algorithm": "balanced_digest_rank_v1",
        "extra_window_episode_identities": [
            ["fixture/canonical", "sid", "revision", "data.parquet", index]
            for index in range(remainder)
        ],
        "observation_count": observation_count,
        "action_evaluable_observation_count": observation_count,
        "valid_action_element_count": possible_elements,
        "possible_action_element_count": possible_elements,
        "valid_action_fraction": 1.0,
        "valid_action_channel_counts": [
            observation_count * action_horizon
        ]
        * action_dim,
        "zero_valid_action_episodes": [],
        ...
        "window_selection_sha256": "9" * 64,
        "canonical_eval_manifest_sha256": manifest_sha,
        ...
        "episode_split_provenance": [
            {
                ...
                "selected_episode_set_sha256": "b" * 64,
                "train_episode_set_sha256": "c" * 64,
                "holdout_episode_set_sha256": "d" * 64,
                "full_catalog_sha256": "e" * 64,
                "train_statistics_sha256": "f" * 64,
                "selected_episode_count": holdout_episode_count,
            }
        ],
    }
    if remainder == 0:
...
def _run_evidence(
    run_dir: Path,
    *,
    effective_global_batch_size: int = 2,
    ...
    if canonical:
        unbiased = _canonical_report(
            observation_count=(
                evaluation_observation_count
                if evaluation_observation_count is not None
                else effective_global_batch_size
            )
        )
        focused = None
    else:
    ...
    if focused is not None:
        (run_dir / "heldout_focused_eval_windows.json").write_text(
            json.dumps(focused)
        )
...
def _canonical_eval_artifact(run_dir: Path, manifest, *, value=0.125):
    evidence = _run_evidence(
        run_dir,
        effective_global_batch_size=208,
    ...
    payload = {
        ...
        "checkpoint": {
            ...
            "source_kind": "checkpoint",
            "source_path": str(
                run_dir / "checkpoints" / f"steps_{step}"
            ),
            "trainer_state_sha256": manifest.file(
                "trainer_state.json"
            ).sha256,
            "model_file": "model.safetensors",
            "model_file_size_bytes": manifest.file(
                "model.safetensors"
            ).size,
            "model_file_sha256": manifest.file("model.safetensors").sha256,
        },
        "run": {
            "run_id": RUN_ID,
            "output_dir": str(run_dir),
        ...
        # Current writers retain an empty focused metrics object; the mirror
        # also accepts its omission for canonical unbiased-only artifacts.
        "metrics": {
            "unbiased": {CANONICAL_METRIC: value},
            "focused": {},
        },
    }
    path.write_text(json.dumps(payload), encoding="utf-8")
...
def test_run_evidence_uses_eval_count_independent_of_training_global_batch(
    tmp_path,
):
    run_dir = tmp_path / RUN_ID
    evidence = _run_evidence(
    ...
    assert evidence.unbiased_sampling_report["observation_count"] == 128
    assert evidence.focused_sampling_report is not None
    assert evidence.focused_sampling_report["observation_count"] == 128


def test_canonical_unbiased_only_evidence_and_eval_artifact_validate(tmp_path):
    run_dir = tmp_path / RUN_ID
    manifest = _manifest(
        run_dir,
        10,
    ...
    artifact_path, evidence = _canonical_eval_artifact(
        run_dir,
        manifest,
        value=0.125,
    )

    assert evidence.focused_sampling_report is None
    assert set(evidence.sampling_reports()) == {"unbiased"}
    artifact = mirror.validate_eval_artifact(
        artifact_path,
        checkpoint_manifest=manifest,
    ...
    payload["metrics"].pop("focused")
    artifact_path.write_text(json.dumps(payload))
    assert mirror.validate_eval_artifact(
        artifact_path,
        checkpoint_manifest=manifest,
        metric_name=CANONICAL_METRIC,
        metric_mode=MODE,
...
def test_modern_uniform_window_allocation_and_digest_validate():
    unbiased, focused = _modern_production_reports((2, 2, 2, 2))
    mirror._validate_window_report_contracts(
        unbiased, focused, expected_observations=8
    )

    tampered = copy.deepcopy(unbiased)
    tampered["windows"][0]["base_index"] += 1
    with pytest.raises(
        mirror.MirrorError,
        match="duplicate heldout window|canonical producer order|window_selection_sha256",
    ):
        mirror._validate_window_report_contracts(
            tampered, focused, expected_observations=8
        )


def test_modern_remainder_window_allocation_validates_exact_qr_counts():
    unbiased, focused = _modern_production_reports((3, 3, 2, 2))
    mirror._validate_window_report_contracts(
        unbiased, focused, expected_observations=10
    )

    for report in (unbiased, focused):
        assert report["base_frames_per_episode"] == 2
        assert report["extra_window_episode_count"] == 2
        assert report["extra_window_episode_identities"] == [
            {"dataset_name": "fixture", "episode_id": 0},
            {"dataset_name": "fixture", "episode_id": 1},
        ]
        assert "frames_per_episode" not in report
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda report: report["extra_window_episode_identities"].__setitem__(
                0, {"dataset_name": "fixture", "episode_id": 3}
            ),
            "per-episode window counts|extra-window identities",
        ),
        (
            lambda report: report["windows"].pop(),
            "cardinality",
        ),
        (
            lambda report: report.update(holdout_episode_count=5),
            "allocation",
        ),
        (
            lambda report: report.update(frames_per_episode=None),
            "cannot claim a scalar",
        ),
        (
            lambda report: report["episode_split_provenance"][0].update(
                holdout_episode_count=3
            ),
            "selected/holdout episode counts differ",
        ),
    ],
)
def test_modern_remainder_allocation_tampering_fails_closed(mutation, message):
    unbiased, focused = _modern_production_reports((3, 3, 2, 2))
    mutation(unbiased)
    _reseal_report_digests(unbiased, focused)
    with pytest.raises(mirror.MirrorError, match=message):
...
def test_checkpoint_contract_authenticates_post_resume_lineage(tmp_path):
    checkpoint = _checkpoint(
        tmp_path / "run",
        20,
        best_step=20,
    ...
    payload = _add_resume_invocation(checkpoint, resumed_from_step=10)
    ...
    assert manifest.file("resume_invocation.yaml").sha256 == hashlib.sha256(
        payload
    ).hexdigest()
    assert manifest.checkpoint_schema == "selection_v1"
...
@pytest.mark.parametrize(
    ("with_resume_invocation", "with_data_stream_state"),
    (
        (False, False),
        (True, False),
        (False, True),
...
def test_checkpoint_contract_accepts_optional_provenance_cross_product(
    tmp_path,
    with_resume_invocation,
    with_data_stream_state,
):
    checkpoint = _checkpoint(
        tmp_path
        / f"resume-{int(with_resume_invocation)}"
        / f"stream-{int(with_data_stream_state)}"
        / "run",
        20,
        best_step=20,
        best_value=0.2,
    )
    if with_resume_invocation:
        _add_resume_invocation(checkpoint, resumed_from_step=10)
    if with_data_stream_state:
        _add_data_stream_state(checkpoint)
    ...
    assert (
        "resume_invocation.yaml" in manifest_paths
    ) is with_resume_invocation
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("local_batch_offset_in_epoch", 11, "offset exceeds"),
        ("optimizer_steps_completed", 19, "does not bind"),
        ("epoch_schedule_sha256", "A" * 64, "lowercase SHA-256"),
        ("checkpoint_boundary", "microbatch", "checkpoint_boundary"),
    ],
)
@pytest.mark.parametrize("with_resume_invocation", (False, True))
def test_checkpoint_contract_rejects_corrupt_exact_datastream_cursor(
    tmp_path,
    field,
    value,
    message,
    ...
    checkpoint = _checkpoint(
        tmp_path / f"resume-{int(with_resume_invocation)}" / "run",
        20,
    )
    if with_resume_invocation:
        _add_resume_invocation(checkpoint, resumed_from_step=10)
    ...
    trainer_state_path = checkpoint / "trainer_state.json"
    trainer_state = json.loads(trainer_state_path.read_text(encoding="utf-8"))
    trainer_state["data_stream_state"][field] = value
    trainer_state_path.write_text(
        json.dumps(trainer_state),
        encoding="utf-8",
    )
    ...
    with pytest.raises(mirror.MirrorError, match=message):
        mirror.validate_checkpoint_tree(
            checkpoint,
            metric_name=METRIC,
            metric_mode=MODE,
        )
...
def test_checkpoint_contract_rejects_extra_exact_datastream_field(tmp_path):
    checkpoint = _checkpoint(tmp_path / "run", 20)
    _add_data_stream_state(checkpoint)
    trainer_state_path = checkpoint / "trainer_state.json"
    trainer_state = json.loads(trainer_state_path.read_text(encoding="utf-8"))
    trainer_state["data_stream_state"]["surprise"] = 1
    trainer_state_path.write_text(
        json.dumps(trainer_state),
        encoding="utf-8",
    )

    with pytest.raises(mirror.MirrorError, match="incomplete or unexpected"):
        mirror.validate_checkpoint_tree(
            checkpoint,
            metric_name=METRIC,
            metric_mode=MODE,
...
@pytest.mark.parametrize(
    ...
    (
        "missing_sidecar",
        "tampered_sidecar",
        "sidecar_without_lineage",
        "future_resume_step",
        "non_boolean_runtime_flag",
    ),
)
def test_checkpoint_contract_rejects_invalid_resume_lineage(tmp_path, mutation):
    checkpoint = _checkpoint(tmp_path / mutation / "run", 20)
    _add_resume_invocation(checkpoint, resumed_from_step=10)
    trainer_state_path = checkpoint / "trainer_state.json"
    trainer_state = json.loads(trainer_state_path.read_text(encoding="utf-8"))

    if mutation == "missing_sidecar":
        (checkpoint / "resume_invocation.yaml").unlink()
    elif mutation == "tampered_sidecar":
        (checkpoint / "resume_invocation.yaml").write_bytes(b"tampered\n")
    elif mutation == "sidecar_without_lineage":
        trainer_state.pop("resume_invocation")
        trainer_state_path.write_text(json.dumps(trainer_state), encoding="utf-8")
    elif mutation == "future_resume_step":
        trainer_state["resume_invocation"]["resumed_from_step"] = 21
        trainer_state_path.write_text(json.dumps(trainer_state), encoding="utf-8")
    elif mutation == "non_boolean_runtime_flag":
        trainer_state["resume_invocation"]["runtime_override_required"] = 1
        trainer_state_path.write_text(json.dumps(trainer_state), encoding="utf-8")
    else:
        raise AssertionError(mutation)

    with pytest.raises(mirror.MirrorError, match="resume_invocation"):
        mirror.validate_checkpoint_tree(
            checkpoint,
            metric_name=METRIC,
            metric_mode=MODE,
...
def test_checkpoint_contract_still_rejects_arbitrary_extra_files(tmp_path):
    checkpoint = _checkpoint(tmp_path / "run", 20)
    _add_resume_invocation(checkpoint, resumed_from_step=10)
    (checkpoint / "unreviewed.txt").write_text("unexpected\n", encoding="utf-8")

    with pytest.raises(mirror.MirrorError, match="unexpected=.*unreviewed"):
        mirror.validate_checkpoint_tree(
            checkpoint,
            metric_name=METRIC,
            metric_mode=MODE,
...
def test_lightweight_checkpoint_contract_accepts_resume_sidecar_only(tmp_path):
    run_dir = tmp_path / RUN_ID
    checkpoint = _checkpoint(run_dir, 10)
    _add_resume_invocation(checkpoint, resumed_from_step=5)
    inventory = mirror.lightweight_source_inventory(run_dir)
    ...
    assert not complete
    assert "unreviewed.txt" in str(reason)
