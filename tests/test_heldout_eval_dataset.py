    ...
    def __init__(self):
        ...
        self.metadata = SimpleNamespace(
            modalities=SimpleNamespace(
                ...
                state={
                    "source": SimpleNamespace(shape=(19,))
                },
            ),
...
def _manifest(tmp_path, *, action_max=4, frames_per_episode=1):
...
def _balanced_manifest(tmp_path, *, action_max=4):
    path = tmp_path / "balanced_split.json"
    path.write_text(
        json.dumps(
            {
                "evaluation_sampling": {
                    ...
                    "window_allocation": {
                        ...
                        "extra_window_episode_identities": [
                            {"dataset_name": "fixture", "episode_id": 7}
                        ],
                    },
                    "seed_sha256": "ab" * 32,
                    "observation_mode": "deployment_action_current_qwen_rgb_v1",
...
def test_multiple_distinct_windows_per_manifest_episode(tmp_path):
    manifest = _manifest(tmp_path, frames_per_episode=4)
    first = DeterministicHeldoutEvalDataset.from_manifest(
        _mixture(), manifest, action_dim=2
    )
    second = DeterministicHeldoutEvalDataset.from_manifest(
    ...
    assert len(first) == 8
    assert first.manifest_holdout_episode_count == 2
    assert first.heldout_window_references == second.heldout_window_references
    for episode_id in (7, 9):
        base_indices = [
            reference.base_index
            for reference in first.heldout_window_references
            if reference.episode_id == episode_id
        ]
        assert len(base_indices) == 4
        assert len(set(base_indices)) == 4


def test_balanced_remainder_selects_exact_explicit_window_allocation(tmp_path):
    manifest = _balanced_manifest(tmp_path)
    first = DeterministicHeldoutEvalDataset.from_manifest(
        _mixture(), manifest, action_dim=2
    )
    second = DeterministicHeldoutEvalDataset.from_manifest(
    ...
    counts = {
        episode_id: sum(
            reference.episode_id == episode_id
            for reference in first.heldout_window_references
        )
        for episode_id in (7, 9)
    }
    assert counts == {7: 3, 9: 2}
    assert all(
        len(
            {
                reference.base_index
    ...
    contract = first.evaluation_sampling_contract
    assert contract.observation_count == 5
    assert contract.holdout_episode_count == 2
    assert contract.frames_per_episode is None
    assert contract.windows_for_episode("fixture", 7) == 3
    assert contract.windows_for_episode("fixture", 9) == 2
    report = first.sampling_report()
    assert report["observation_count"] == 5
    assert report["holdout_episode_count"] == 2
    assert report["base_frames_per_episode"] == 2
    assert report["extra_window_episode_count"] == 1
    assert report["maximum_frames_per_episode"] == 3
    assert "frames_per_episode" not in report


def test_balanced_remainder_focused_view_uses_same_episode_multiplicities(tmp_path):
    dataset = DeterministicHeldoutEvalDataset.from_manifest(
        _focused_mixture(),
        _balanced_manifest(tmp_path, action_max=49),
        action_dim=18,
        focused_subtasks=(2, 3),
    )
    focused = dataset.make_focused_view()
    assert len(dataset) == len(focused) == 5
    assert Counter(
        reference.episode_id for reference in focused.heldout_window_references
    ) == {7: 3, 9: 2}
    assert all(
        len(
            {
                reference.base_index
                for reference in focused.heldout_window_references
                if reference.episode_id == episode_id
            }
        )
        == count
        for episode_id, count in {7: 3, 9: 2}.items()
    )


def test_balanced_remainder_contract_fails_closed_on_unknown_extra_episode(tmp_path):
    payload = json.loads(_balanced_manifest(tmp_path).read_text(encoding="utf-8"))
    payload["evaluation_sampling"]["window_allocation"][
        "extra_window_episode_identities"
    ][0]["episode_id"] = 11
    path = tmp_path / "invalid_balanced_split.json"
    path.write_text(json.dumps(payload), encoding="utf-8")
    with pytest.raises(ValueError, match="not heldout episodes"):
        load_evaluation_sampling_contract(path)
...
def test_eval_sampling_contract_and_effective_batch_count_fail_closed(tmp_path):
    ...
    assert validate_global_eval_observation_count(
        ...
        per_device_batch_size=16,
        world_size=8,
        # Explicit eval cardinality is deliberately independent of training
...
def test_eval_cache_config_is_independent_and_capped_at_one():
    train_cfg = OmegaConf.create(
        {
            "lerobot_v3_parquet_cache_size": 5,
            "video_target_shift_steps": 2,
            "episode_split_manifest": "/holdout.json",
            "normalization_statistics_artifact": "/union-stats.json",
            "frozen_train_view_manifest": "/train-view.json",
    ...
    assert eval_cfg.episode_split_manifest == "/holdout.json"
    assert eval_cfg.normalization_statistics_artifact == "/union-stats.json"
    assert eval_cfg.frozen_train_view_manifest is None
    assert eval_cfg.frozen_train_view_manifest_sha256 is None
    assert eval_cfg.frozen_train_view_require_data_shard_hashes is False
    assert train_cfg.frozen_train_view_manifest == "/train-view.json"
    assert train_cfg.frozen_train_view_manifest_sha256 == "a" * 64
    assert train_cfg.frozen_train_view_require_data_shard_hashes is True
...
def test_focused_view_selects_multiple_distinct_windows_per_episode(tmp_path):
    dataset = DeterministicHeldoutEvalDataset.from_manifest(
        _focused_mixture(),
        _manifest(tmp_path, action_max=49, frames_per_episode=4),
        action_dim=18,
        focused_subtasks=(2, 3),
    )
    focused = dataset.make_focused_view()

    assert len(dataset) == len(focused) == 8
    for episode_id in (7, 9):
        base_indices = [
            reference.base_index
            for reference in focused.heldout_window_references
        ...
        assert len(base_indices) == 4
        assert len(set(base_indices)) == 4
    assert focused.sampling_report()["frames_per_episode"] == 4
...
def test_focused_movement_diagnostics_use_each_candidate_delta_anchor():
    mixture = _focused_mixture()
    dataset = mixture.datasets[0]
    frame = dataset._frames[7]
    states = np.zeros((70, 18), dtype=np.float32)
    states[:] = np.arange(70, dtype=np.float32)[:, None]
    actions = states.copy()
    actions[:, [7, 15]] = 1.0
    states[:, [7, 15]] = 1.0
    frame["source.action"] = list(actions)
    frame["source.observation.state"] = list(states)
    dataset.lerobot_modality_meta.state["source"] = SimpleNamespace(
        original_key="source.observation.state",
        start=0,
        end=18,
    )
    dataset.metadata.modalities.state = {"source": SimpleNamespace(shape=(18,))}
    dataset.transforms = AnchorRelativeActionTransform(
        apply_to=["action.source_controls"],
        mappings={
            "source_controls": {
                "state_key": "source",
                "state_indices": list(range(18)),
                "delta_mask": (
                    [True] * 7
                    + [False]
                    + [True] * 7
    ...
    # The target at the anchor is a zero delta; steps 2..9 exceed the
    # threshold on all 14 arm channels.
    assert diagnostic.arm_movement_elements_h10 == 8 * 14
    assert diagnostic.arm_movement_hold_abs_h10 == pytest.approx(
        14 * sum(range(2, 10))
    )
