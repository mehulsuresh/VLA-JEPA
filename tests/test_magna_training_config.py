...
def test_magna_production_config_contract():
    ...
    assert action_cfg.state_dim == 18
    ...
    assert data_cfg.action_type == "joint_delta_gripper_absolute"
    assert data_cfg.action_delta_anchor == "chunk_start_state"
    assert data_cfg.gripper_action_type == "absolute"
    assert data_cfg.state_action_normalization == "q01_q99_unclipped"
    assert list(data_cfg.action_delta_mappings.source_controls.delta_mask) == (
        [True] * 7 + [False] + [True] * 7 + [False]
    )
    assert list(data_cfg.action_delta_mappings.source_head.delta_mask) == [True, True]
    assert data_cfg.modality_metadata_overrides.state.source.original_key == (
        "source.observation.state"
    )
    assert (data_cfg.modality_metadata_overrides.state.source.start, data_cfg.modality_metadata_overrides.state.source.end) == (0, 18)
    ...
    assert data_cfg.append_subtask_to_prompt is True
    assert data_cfg.subtask_prompt_source_column == "subtask_index"
    assert data_cfg.subtask_prompt_label_column == "local_subtask_text"
    assert data_cfg.subtask_prompt_append_probability == 0.7
    assert cfg.trainer.allow_compile_with_deepspeed is False
    assert data_cfg.subtask_prompt_separator == " | "
    assert data_cfg.replace_modality_metadata_with_overrides is True
...
def test_ogrealman_human_labelled_config_uses_explicit_subtask_prompt_contract():
    cfg = OmegaConf.load(OGREALMAN_HUMAN_LABELLED_CONFIG_PATH)
    data_cfg = cfg.datasets.vla_data
    ...
    assert data_cfg.subtask_prompt_append_probability == 0.7
    assert data_cfg.subtask_prompt_separator == " | "
    assert list(data_cfg.subtask_prompt_ignored_labels) == ["__unlabeled__"]


def test_every_enabled_subtask_training_config_uses_seventy_percent():
    enabled_configs = []
    for config_path in sorted(TRAINING_CONFIG_DIR.glob("*.yaml")):
        cfg = OmegaConf.load(config_path)
        data_cfg = OmegaConf.select(cfg, "datasets.vla_data")
        if data_cfg is None or not bool(
            OmegaConf.select(data_cfg, "append_subtask_to_prompt", default=False)
        ):
            continue
        enabled_configs.append(config_path.name)
        assert (
            OmegaConf.select(
                data_cfg,
    ...
    assert {
        ...
        "vlajepa_robot_ft_lerobot_magna_interventions_a100x8_qwen35_2b_full_moge_vitb_vjepa_large.yaml",
        "vlajepa_robot_ft_lerobot_magna_interventions_h100x8_b16_qwen35_2b_full_moge_vitb_vjepa_large.yaml",
        "vlajepa_robot_ft_lerobot_ogrealman_human_labelled_cloud_a100x8_qwen35_2b_full_moge_vitb_vjepa_large.yaml",
    }.issubset(enabled_configs)
...
def test_magna_mixture_and_launcher_route_to_production_config():
    ...
    assert "STARVLA_CONFIG_IS_AUTHORITATIVE" in launcher


def test_a100_production_launcher_cannot_disable_config_authority(tmp_path):
    env = os.environ.copy()
    env.update(
        {
            "ACCELERATE_BIN": "/bin/echo",
            "RUN_ID": "must_reject_legacy_mode",
            "STARVLA_CONFIG_IS_AUTHORITATIVE": "0",
            "STARVLA_USE_DEEPSPEED": "0",
            "VLA_JEPA_SCRATCH": str(tmp_path),
        }
    )
    ...
    assert "requires STARVLA_CONFIG_IS_AUTHORITATIVE=1" in result.stderr


def test_authoritative_launcher_does_not_translate_training_environment(tmp_path):
    env = os.environ.copy()
    env.update(
        {
            "ACCELERATE_BIN": "/bin/echo",
            "CONFIG_YAML": str(CONFIG_PATH),
            "RUN_ID": "config_authoritative_test",
            "STARVLA_CONFIG_IS_AUTHORITATIVE": "1",
            "STARVLA_USE_DEEPSPEED": "0",
            "VLA_JEPA_SCRATCH": str(tmp_path),
            "DATA_ROOT_DIR": "/stale/dataset",
    ...
    training_start = command.index("./starVLA/training/train_starvla.py") + 1
    assert command[training_start:] == [
        "--config_yaml",
        str(CONFIG_PATH),
        "--run_id",
        "config_authoritative_test",
    ]


def test_authoritative_launcher_rejects_cli_training_override(tmp_path):
    env = os.environ.copy()
    env.update(
        {
            "ACCELERATE_BIN": "/bin/echo",
    ...
    result = subprocess.run(
        ["bash", str(BASE_LAUNCHER_PATH), "--trainer.epochs", "99"],
        check=False,
        env=env,
        text=True,
        capture_output=True,
    ...
    assert result.returncode == 2
    assert "refuses training override" in result.stderr
...
def test_clean_pilot_pins_config_budget_and_clean_invariants(tmp_path):
    ...
    for option in (
        "--datasets.vla_data.append_task_id_to_prompt",
        "--datasets.vla_data.append_subtask_to_prompt",
        "--datasets.vla_data.task_id_prompt_append_probability",
        "--datasets.vla_data.subtask_prompt_source_column",
        "--datasets.vla_data.subtask_prompt_label_column",
        "--datasets.vla_data.subtask_prompt_append_probability",
        "--datasets.vla_data.subtask_prompt_separator",
        "--datasets.vla_data.subtask_prompt_ignored_labels",
    ):
