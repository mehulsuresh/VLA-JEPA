...
def test_filtering_precedes_dense_steps_and_prunes_v3_episode_metadata(tmp_path):
    ...
    dataset._subtask_metadata_path = root / "meta/subtasks.parquet"
    dataset._subtask_metadata_sha256 = "subtask-sha256"
    dataset._subtask_labels = {0: "__unlabeled__", 1: "grasp chain"}
    ...
    assert provenance["subtask_prompt"] == {
        "enabled": True,
        "source_column": "subtask_index",
        "label_column": "local_subtask_text",
        "append_probability": 1.0,
        "separator": " | ",
        "ignored_labels": ["__unlabeled__"],
        "metadata_path": str(dataset._subtask_metadata_path.resolve()),
        "metadata_sha256": "subtask-sha256",
        "label_count": 2,
        "usable_label_count": 1,
        "selected_source_ids": [],
        "selected_frame_count": 0,
        "selected_usable_frame_count": 0,
        "selected_assignment_sha256": None,
    }


def test_dataset_provenance_is_immutable_for_existing_run(tmp_path):
    provenance = {"dataset_name": "unit", "subtask_prompt": {"metadata_sha256": "a"}}
    dataset = SimpleNamespace(dataset_provenance=lambda: provenance)
    mixture = object.__new__(LeRobotMixtureDataset)
    mixture.datasets = [dataset]
    provenance_path = tmp_path / "dataset_provenance.json"

    mixture.save_dataset_provenance(provenance_path)
    original_bytes = provenance_path.read_bytes()
    mixture.save_dataset_provenance(provenance_path)

    provenance["subtask_prompt"]["metadata_sha256"] = "b"
