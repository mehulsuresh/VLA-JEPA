...
def test_retention_preserves_intended_milestones_before_rotating_recovery_points(
    tmp_path,
):
    trainer = _trainer(tmp_path, checkpoint_max_to_keep=3)
    trainer.config.trainer.max_train_steps = 40
    trainer.config.trainer.checkpoint_eval_milestone_steps = [10, 20]
    for step in (10, 20, 30, 40):
        _checkpoint(trainer, step, best_step=10, best_value=0.25)
    trainer.completed_steps = 40
    trainer.best_metric_value = 0.25
    trainer.best_metric_step = 10
