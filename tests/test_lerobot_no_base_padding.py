...
def test_lerobot_subtask_prompt_uses_configured_source_and_keeps_raw_label(tmp_path):
    dataset = object.__new__(LeRobotSingleDataset)
    dataset._subtask_labels = {7: "lift the chain"}
    dataset.data_cfg = {
        "append_subtask_to_prompt": True,
        "subtask_prompt_source_column": "local_stage_id",
        "subtask_prompt_append_probability": 0.0,
        "subtask_prompt_separator": " :: ",
        "append_task_id_to_prompt": False,
    }
    label_row = pd.Series({"local_stage_id": 7, "task_id": 42})

    language, subtask_label, task_id_label = dataset._append_prompt_labels_for_row(
        "move the chain",
        label_row,
    )

    assert language == "move the chain"
    assert subtask_label == "lift the chain"
    assert task_id_label is None


def test_lerobot_subtask_and_task_id_prompting_are_independent():
    dataset = object.__new__(LeRobotSingleDataset)
    dataset._subtask_labels = {7: "lift the chain"}
    dataset.data_cfg = {
        "append_subtask_to_prompt": True,
        ...
        "task_id_prompt_source_column": "task_id",
        "task_id_label_map": {42: "chain transfer"},
        "task_id_prompt_append_probability": 1.0,
        "task_id_prompt_separator": " | ",
    }
    label_row = pd.Series({"subtask_index": 7, "task_id": 42})
    ...
    assert language == "move the chain | lift the chain | chain transfer"
    assert subtask_label == "lift the chain"
    assert task_id_label == "chain transfer"


def test_lerobot_subtask_metadata_uses_only_configured_label_column(tmp_path):
    meta_dir = tmp_path / "meta"
    meta_dir.mkdir()
    pd.DataFrame(
        {
            "subtask_index": [2],
            "local_subtask_text": ["reach into the bin"],
            "description": ["retrieve the chain"],
        }
    ).to_parquet(meta_dir / "subtasks.parquet")
    ...
    dataset._dataset_path = tmp_path
    dataset._lerobot_version = "v3.0"
    dataset.data_cfg = {"subtask_prompt_label_column": "description"}

    assert dataset._get_subtask_labels() == {2: "retrieve the chain"}

    dataset.data_cfg = {"subtask_prompt_label_column": "missing_column"}
    assert dataset._get_subtask_labels() == {}

    dataset.data_cfg = {
        "append_subtask_to_prompt": True,
        "subtask_prompt_label_column": "missing_column",
    }
    with pytest.raises(ValueError, match="missing configured subtask label column"):
        dataset._get_subtask_labels()


def test_lerobot_enabled_subtask_prompt_rejects_unknown_or_missing_source():
    dataset = object.__new__(LeRobotSingleDataset)
    dataset._dataset_name = "unit"
    dataset._subtask_labels = {2: "reach into the bin"}
    dataset.data_cfg = {
        "append_subtask_to_prompt": True,
        "subtask_prompt_source_column": "subtask_index",
    }

    with pytest.raises(SubtaskPromptDataError, match="missing configured source column"):
        dataset._subtask_prompt_label_for_row(pd.Series({"task_id": 0}))
    with pytest.raises(SubtaskPromptDataError, match="No subtask label mapping"):
        dataset._subtask_prompt_label_for_row(pd.Series({"subtask_index": 99}))
    with pytest.raises(SubtaskPromptDataError, match="No subtask label mapping"):
        dataset._subtask_prompt_label_for_row(pd.Series({"subtask_index": 2.5}))


def test_lerobot_mixture_does_not_retry_subtask_prompt_data_errors():
    mixture = object.__new__(LeRobotMixtureDataset)
    calls = 0

    def invalid_sample(_index):
        nonlocal calls
        calls += 1
        raise SubtaskPromptDataError("bad subtask mapping")

    mixture.sample_step = invalid_sample

    with pytest.raises(SubtaskPromptDataError, match="bad subtask mapping"):
        mixture[0]
    assert calls == 1


def test_lerobot_rejects_legacy_task_id_as_subtask_configuration():
    dataset = object.__new__(LeRobotSingleDataset)
    dataset.data_cfg = {
        "append_task_id_to_prompt": True,
        "task_id_prompt_source_column": "subtask_index",
    }

    with pytest.raises(ValueError, match="use append_subtask_to_prompt"):
        dataset._validate_subtask_prompt_schema()


def test_lerobot_subtask_prompt_validates_selected_split_coverage(tmp_path):
    data_path = tmp_path / "data/chunk-000/file-000.parquet"
    data_path.parent.mkdir(parents=True)
    pd.DataFrame(
        {
            "episode_index": [10, 10, 20, 20],
            "subtask_index": [0, 0, 0, 2],
        }
    ...
    dataset._subtask_labels = {0: "__unlabeled__", 2: "grasp chain"}
    dataset.data_cfg = {
        "append_subtask_to_prompt": True,
        "subtask_prompt_source_column": "subtask_index",
    ...
    dataset.trajectory_ids_to_metadata = {
        10: {"data/chunk_index": 0, "data/file_index": 0},
        20: {"data/chunk_index": 0, "data/file_index": 0},
    }

    dataset._trajectory_ids = np.asarray([10], dtype=np.int64)
    dataset._trajectory_lengths = np.asarray([2], dtype=np.int64)
    with pytest.raises(ValueError, match="zero frames with a usable"):
        dataset._validate_selected_subtask_prompt_coverage()

    dataset._trajectory_ids = np.asarray([10, 20], dtype=np.int64)
    dataset._trajectory_lengths = np.asarray([2, 2], dtype=np.int64)
    dataset._validate_selected_subtask_prompt_coverage()
    assert dataset._selected_subtask_source_ids == [0, 2]
    assert dataset._selected_subtask_prompt_frame_count == 4
    assert dataset._selected_subtask_prompt_usable_frame_count == 1
    original_assignment_sha256 = dataset._selected_subtask_assignment_sha256
    assert len(original_assignment_sha256) == 64

    pd.DataFrame(
        {
            "episode_index": [10, 10, 20, 20],
            "subtask_index": [0, 0, 2, 0],
        }
    ).to_parquet(data_path)
    dataset._validate_selected_subtask_prompt_coverage()
    assert dataset._selected_subtask_prompt_usable_frame_count == 1
    assert dataset._selected_subtask_assignment_sha256 != original_assignment_sha256


def test_lerobot_subtask_metadata_rejects_fractional_ids_when_enabled(tmp_path):
    meta_dir = tmp_path / "meta"
    meta_dir.mkdir()
    pd.DataFrame(
        {
            "subtask_index": [2.5],
            "local_subtask_text": ["invalid fractional ID"],
        }
    ).to_parquet(meta_dir / "subtasks.parquet")
    dataset = object.__new__(LeRobotSingleDataset)
    dataset._dataset_path = tmp_path
    ...
    with pytest.raises(ValueError, match="invalid subtask_index 2.5"):
        dataset._get_subtask_labels()
...
@pytest.mark.parametrize(
    ("trajectory", "modality_keys", "delta_indices", "action_shape", "message"),
    (
        (
            None,
            {"action": ["action.source_controls"]},
            {"action.source_controls": np.arange(4)},
            (4, 19),
            "trajectory metadata is unavailable",
        ),
        (
            pd.DataFrame({"valid_state": [1, 1, 1, 1]}),
            {},
            {},
            (4, 19),
            "action modality metadata is unavailable",
        ),
        (
            pd.DataFrame({"valid_state": [1, 1, 1, 1]}),
            {"action": ["action.source_controls"]},
            {"action.source_controls": np.arange(3)},
            (4, 19),
            "action-offset count",
        ),
        (
            ...
            {"action.source_controls": np.arange(4)},
            (4, 19),
            "trajectory metadata is empty",
        ),
    ),
)
def test_lerobot_action_validity_fail_closed_rejects_missing_or_misaligned_metadata(
    trajectory,
    modality_keys,
    delta_indices,
    action_shape,
    ...
    mixture = _minimal_lerobot_mixture_for_action_mask()
    mixture.action_validity_fail_closed = True
    dataset = object.__new__(LeRobotSingleDataset)
    dataset.curr_traj_data = trajectory
    dataset._modality_keys = modality_keys
    dataset._delta_indices = delta_indices
    dataset.data_cfg = {
        "use_action_validity_prefix_mask": True,
        "action_validity_label_key": "valid_state",
        "action_validity_positive_is_valid": True,
    ...
    with pytest.raises(ValueError, match=message):
        mixture._build_action_validity_mask(
            dataset,
            step=0,
...
@pytest.mark.parametrize(
    ("frame", "message"),
    (
        (pd.DataFrame({"frame_index": [0, 1, 2, 3]}), "column.*missing"),
        (
            pd.DataFrame({"valid_state": [1.0, np.nan, 1.0, 1.0]}),
            "contains NaN/Inf",
        ),
    ),
)
def test_lerobot_action_validity_fail_closed_rejects_bad_labels(
    frame,
    message,
):
    mixture = _minimal_lerobot_mixture_for_action_mask()
...
def test_lerobot_step_cache_key_ignores_prompt_only_settings(tmp_path):
    dataset = _minimal_lerobot_dataset(tmp_path)
    original_key = dataset._get_steps_config_key()
    ...
    dataset.data_cfg.update(
        {
            ...
            "subtask_prompt_source_column": "subtask_index",
            "subtask_prompt_label_column": "local_subtask_text",
            "subtask_prompt_append_probability": 0.3,
            "subtask_prompt_separator": " <stage> ",
            "subtask_prompt_ignored_labels": ["__unlabeled__"],
        }
    )

    assert dataset._get_steps_config_key() == original_key
