...
def test_explicit_null_holdout_sampling_uses_legacy_global_batch_count():
    cfg = OmegaConf.create({"holdout_sampling": None})

    assert (
        _resolve_heldout_eval_observation_count(
            cfg,
            default_observation_count=128,
        )
        == 128
    )
...
@pytest.mark.parametrize(
    ...
    (
        [],
        "not-a-policy",
        {"evaluation_observation_count": True},
        {"evaluation_observation_count": 0},
        {"evaluation_observation_count": 1.5},
    ),
)
def test_invalid_runtime_holdout_sampling_fails_closed(holdout_sampling):
    cfg = OmegaConf.create({"holdout_sampling": holdout_sampling})

    with pytest.raises(
        ValueError,
        match=(
            "holdout_sampling must be an object or null|"
...
def _balanced_coverage_report(
    *,
    ...
    report = {
        ...
        "zero_valid_action_episodes": [],
        "episode_split_provenance": _split_provenance(
        ...
        "extra_window_episode_identities": [
            ["fixture", index]
            for index in range(extra_window_episode_count)
        ],
    }
    if extra_window_episode_count == 0:
        report["frames_per_episode"] = base_frames_per_episode
    return report


@pytest.mark.parametrize(
    ("holdout_episode_count", "base_frames", "extra_windows"),
    ((8, 16, 0), (56, 2, 16)),
)
def test_balanced_eval_report_coverage_accepts_uniform_and_remainder(
    holdout_episode_count,
    base_frames,
    extra_windows,
):
    report = _balanced_coverage_report(
        holdout_episode_count=holdout_episode_count,
        base_frames_per_episode=base_frames,
        extra_window_episode_count=extra_windows,
    )
    ...
    _validate_heldout_report_coverage(
        ...
        required_subtasks=(),
        minimum_per_subtask=1,
        label="Balanced heldout eval",
    )


def test_balanced_eval_report_rejects_uniform_alias_on_remainder():
    report = _balanced_coverage_report(
        holdout_episode_count=56,
        base_frames_per_episode=2,
        extra_window_episode_count=16,
    )
    report["frames_per_episode"] = 2

    with pytest.raises(ValueError, match="remainder allocation must omit"):
        _validate_heldout_report_coverage(
            report,
            expected_observations=128,
            required_subtasks=(),
