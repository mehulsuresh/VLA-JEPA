...
def test_policy_metadata_describes_18d_realman_expansion_contract(tmp_path):
    ...
    action_contract = metadata["realman_action_contract"]
    assert action_contract["version"] == 3
    assert action_contract["representation"] == REALMAN_18D_ACTION_CONTRACT.to_dict()
    assert action_contract["representation_sha256"] == REALMAN_18D_ACTION_CONTRACT.sha256()
    assert action_contract["normalization_statistics_sha256"] == metadata[
        "normalization_statistics_sha256"
    ]
    assert action_contract["omitted_robot_action_indices"] == [16, 17, 18, 21]
    assert action_contract["base_velocity_source"] == "zero"
    assert action_contract["lift_source"] == "measured_state"
    input_contract = metadata["realman_input_contract"]
    assert input_contract["state_shape"] == [1, 1, 18]
    assert input_contract["state_normalization_mode"] == Q01_Q99_UNCLIPPED
    assert input_contract["state_normalization_clip"] is False
    assert input_contract["representation_sha256"] == REALMAN_18D_ACTION_CONTRACT.sha256()
    assert input_contract["normalization_statistics_sha256"] == metadata[
        "normalization_statistics_sha256"
    ]
...
def test_realman_metadata_refuses_to_guess_normalization_mode(tmp_path, monkeypatch):
    class Policy:
        config = {
            ...
            "framework": {
                "action_model": {"action_dim": 18, "state_dim": 18, "action_horizon": 50},
                "vj2_model": {"num_frames": 8},
            },
        }
        norm_stats = {
            "new_embodiment": {
                ...
                "state": {"min": [-1.0] * 18, "max": [1.0] * 18},
            }
        }

    monkeypatch.setattr(checkpoint_utils, "_infer_norm_mode_hints", lambda _policy: {})

    with pytest.raises(RuntimeError, match="incompatible delta checkpoint"):
